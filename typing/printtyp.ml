...
module Namespace = struct
  ...
  let best_class_namespace = function
    ...
    | Pident c -> (
        match location Class c with Some _ -> Class | None -> Class_type)
end
...
module Conflicts = struct
  ...
  let pp_explanation ppf r =
    Format.fprintf ppf "@[<v 2>%a:@,Definition of %s %s@]" Location.print_loc
      r.location (Namespace.show r.kind) r.name
  ...
  let print_toplevel_hint ppf l =
    ...
    let pp_submsg ppf names =
      ...
      | [ (namespace, a) ] ->
          Format.fprintf ppf
            "@ @[<2>Hint: The %a %s has been defined multiple times@ in@ this@ \
             toplevel@ session.@ Some toplevel values still refer to@ old@ \
             versions@ of@ this@ %a.@ Did you try to redefine them?@]"
            Namespace.pp namespace a Namespace.pp namespace
      | (namespace, _) :: _ :: _ ->
          Format.fprintf ppf
            "@ @[<2>Hint: The %a %a have been defined multiple times@ in@ \
             this@ toplevel@ session.@ Some toplevel values still refer to@ \
             old@ versions@ of@ those@ %a.@ Did you try to redefine them?@]"
            pp_namespace_plural namespace
            Format.(pp_print_list ~pp_sep:conj pp_print_string)
            (List.map snd names) pp_namespace_plural namespace
    in
...
module Naming_context = struct
  ...
  *)
  ...
  let find_hid id map =
    try (Ident.Map.find id map, map) with Not_found -> add_hid_id id map
  ...
  let pervasives_name namespace name =
    if not !enabled then Out_name.create name
    else
  ...
  let env_ident namespace name =
    if S.mem name !protected then None
    else
  ...
  let ident_name_simple namespace id =
    if not !enabled then Out_name.create (Ident.name id)
    else
      let name = Ident.name id in
      match M.find name (get namespace) with
      | Uniquely_associated_to (id', r) when Ident.same id id' -> r
      ...
      | Uniquely_associated_to (id', r) ->
          ...
          Out_name.set r (human_unique hid' id');
          List.iter
            (fun (id, hid) -> Conflicts.collect_explanation namespace hid id)
            [ (id, hid); (id', hid') ];
          set namespace @@ M.add name (Need_unique_name m) (get namespace);
          Out_name.create (human_unique hid id)
  ...
  let ident_name namespace id =
    (match env_ident namespace (Ident.name id) with
    | Some id' -> ignore (ident_name_simple namespace id')
    | None -> ());
    ident_name_simple namespace id

  let reset () = Array.iteri (fun i _ -> map.(i) <- M.empty) map
end
...
let rec rewrite_double_underscore_paths env p =
  ...
  | Papply (a, b) ->
      Papply
        ( rewrite_double_underscore_paths env a,
  ...
  | Pident id -> (
      ...
      | Some i -> (
          ...
          | p', _ ->
              if module_path_is_an_alias_of env p' ~alias_of:p then p' else p))

let rewrite_double_underscore_paths env p =
  if env == Env.empty then p else rewrite_double_underscore_paths env p

let rec tree_of_path namespace = function
  | Pident id -> Oide_ident (ident_name namespace id)
  ...
  | Pdot (p, s) -> Oide_dot (tree_of_path Module p, s)
  | Papply (p1, p2) ->
...
let path ppf p = !Oprint.out_ident ppf (tree_of_path Other p)
let string_of_path p = Format.asprintf "%a" path p
...
and raw_type_desc ppf = function
  ...
  | Tvariant row ->
      fprintf ppf
        ...
        row.row_closed "row_fixed=" raw_row_fixed row.row_fixed "row_name="
        (fun ppf ->
...
let penalty s =
  if s <> "" && s.[0] = '_' then 10
  else match find_double_underscore s with None -> 1 | Some _ -> 10

let rec path_size = function
  | Pident id -> (penalty (Ident.name id), -Ident.scope id)
  | Pdot (p, _) ->
...
let rec lid_of_path = function
  ...
  | Path.Pdot (p1, s) -> Longident.Ldot (lid_of_path p1, s)
  | Path.Papply (p1, p2) -> Longident.Lapply (lid_of_path p1, lid_of_path p2)
...
let best_type_path p =
  ...
  else if !Clflags.real_paths then (p, Id)
  else
  ...
  if List.mem_assq px !names && not (List.memq px !delayed) then
    ...
    let pr_typ () =
      ...
      | Tconstr (p, tyl, _abbrev) ->
          ...
          else Otyp_constr (tree_of_path Type p', tree_of_typlist sch tyl')
      | Tvariant row -> (
          let row = row_repr row in
          let fields =
            if row.row_closed then
              List.filter
                (fun (_, f) -> row_field_repr f <> Rabsent)
                row.row_fields
            else row.row_fields
          in
          let present =
            List.filter
              (fun (_, f) ->
                match row_field_repr f with Rpresent _ -> true | _ -> false)
              fields
          in
          ...
          | Some (p, tyl) when namable_row row ->
              ...
              if row.row_closed && all_present then out_variant
                ...
                in
                Otyp_variant
                  (non_gen, Ovar_typ out_variant, row.row_closed, tags)
          | _ ->
              let non_gen =
  ...
  | None ->
      let pr_fields fi =
        ...
        let present_fields =
          List.fold_right
            (fun (n, k, t) l ->
              match field_kind_repr k with Fpresent -> (n, t) :: l | _ -> l)
            fields []
        in
        let sorted_fields =
          ...
          if abstr || not is_var then
            let inj =
              ...
              | Some ty ->
                  ...
                  decl.type_private = Private
                  && Btype.is_constr_row ~allow_ident:true
                       (Btype.row_of_type ty)
            and co, cn = Variance.get_upper v in
            ( (if not cn then Covariant
              else if not co then Contravariant
              else NoVariance),
...
let tree_of_extension_constructor id ext es =
  ...
  let name = Ident.name id in
  let args, ret =
    extension_constructor_args_and_ret_type_subtree ext.ext_args
      ext.ext_ret_type
  in
  let ext =
    {
...
let tree_of_class_param param variance =
  ( (match tree_of_typexp true param with Otyp_var (_, s) -> s | _ -> "?"),
...
let class_variance =
  ...
  List.map (fun v ->
      ( (if not (mem May_pos v) then Contravariant
        else if not (mem May_neg v) then Covariant
        else NoVariance),
        NoInjectivity ))
...
let wrap_env fenv ftree arg =
  ...
  if !Clflags.real_paths || same_printing_env env then ()
  ...
  | Sig_type (id, _decl, rs, _) :: rem
    when rs = Trec_first && not !Clflags.real_paths ->
      let rec get_ids = function
        | Sig_type (id, _, Trec_next, _) :: rem -> id :: get_ids rem
        | _ -> []
      in
...
let recursive_sigitem = function
  ...
  | Sig_type (id, _, rs, _) | Sig_module (id, _, _, rs, _) -> Some (id, rs, 0)
  | _ -> None
...
let protect_rec_items items =
  let rec get_ids recs = function
    | [] -> []
    | item :: rem -> (
        match recursive_sigitem item with
...
let rec tree_of_modtype ?(ellipsis = false) = function
  ...
  | Mty_alias p -> Omty_alias (tree_of_path Module p)
...
and trees_of_sigitem = function
  ...
  | Sig_type (id, decl, rs, _) -> [ tree_of_type_declaration id decl rs ]
  | Sig_typext (id, ext, es, _) -> [ tree_of_extension_constructor id ext es ]
  ...
  | Sig_modtype (id, decl, _) -> [ tree_of_modtype_declaration id decl ]
  | Sig_class (id, decl, rs, _) -> [ tree_of_class_declaration id decl rs ]
...
let printed_signature sourcefile ppf sg =
  ...
  if
    Warnings.(is_active @@ Erroneous_printed_signature "")
    && Conflicts.exists ()
  then (
...
let trees_of_type_expansion (t, t') =
  if same_path t t' then (
    ...
    if first = second then Same first else Diff (first, second)
...
let trees_of_type_path_expansion (tp, tp') =
  if Path.same tp tp' then Same (tree_of_path Type tp)
  else Diff (tree_of_path Type tp, tree_of_path Type tp')

let type_path_expansion ppf = function
  | Same p -> !Oprint.out_ident ppf p
  | Diff (p, p') ->
      fprintf ppf "@[<2>%a@ =@ %a@]" !Oprint.out_ident p !Oprint.out_ident p'

let rec trace fst txt ppf = function
  | { Trace.got; expected } :: rem ->
      if not fst then fprintf ppf "@,";
      fprintf ppf "@[Type@;<1 2>%a@ %s@;<1 2>%a@] %a" type_expansion got txt
        type_expansion expected (trace false txt) rem
  | _ -> ()
...
let type_path_list =
  Format.pp_print_list
    ~pp_sep:(fun ppf () -> Format.pp_print_break ppf 2 0)
    type_path_expansion
...
let explanation_diff env t3 t4 : (Format.formatter -> unit) option =
  ...
  | Tarrow (_, ty1, ty2, _), _ when is_unit env ty1 && unifiable env ty2 t4 ->
      Some
        (fun ppf ->
          fprintf ppf "@,@[Hint: Did you forget to provide `()' as argument?@]")
  | _, Tarrow (_, ty1, ty2, _) when is_unit env ty1 && unifiable env t3 ty2 ->
      Some
        (fun ppf ->
          fprintf ppf
            "@,\
...
let explain_fixed_row_case ppf = function
  ...
  | Trace.Cannot_add_tags tags ->
      Format.fprintf ppf "it may not allow the tag(s) %a" print_tags tags

let explain_fixed_row pos expl =
  match expl with
  | Types.Fixed_private ->
...
let explain_variant = function
  ...
  | Trace.No_tags (pos, fields) ->
      Some
        (dprintf "@,@[The %a variant type does not allow tag(s)@ @[<hov>%a@]@]"
           print_pos pos print_tags (List.map fst fields))
  ...
  let pre =
    ...
    | None -> (
        ...
        | Trace.Univ _, Some (Trace.Incompatible_fields { name; diff }) ->
            dprintf
              "@,\
               @[The method %s has type@ %a,@ but the expected method type \
               was@ %a@]"
              name type_expr diff.Trace.got type_expr diff.Trace.expected
        | _ -> ignore)
  in
  match e with
  | Trace.Univ u ->
      Some
        (dprintf "%t@,The universal variable %a would escape its scope" pre
           type_expr u)
  | Trace.Constructor p ->
      Some
        (dprintf "%t@,@[The type constructor@;<1 2>%a@ would escape its scope@]"
           pre path p)
  | Trace.Module_type p ->
      Some
        (dprintf "%t@,@[The module type@;<1 2>%a@ would escape its scope@]" pre
           path p)
  | Trace.Equation (_, t) ->
      Some
        (dprintf "%t @,@[<hov>This instance of %a is ambiguous:@ %s@]" pre
           type_expr t "it would escape the scope of its equation")
  | Trace.Self -> Some (dprintf "%t@,Self type cannot escape its class" pre)
...
let prepare_expansion_head empty_tr = function
  | Trace.Diff d -> Some (Trace.map_diff (may_prepare_expansion empty_tr) d)
  | _ -> None
...
let head_error_printer txt_got txt_but = function
  ...
  | Some d ->
      ...
      dprintf "%t@;<1 2>%a@ %t@;<1 2>%a" txt_got type_expansion d.Trace.got
...
let unification_error env tr txt1 ppf txt2 ty_expect_explanation =
  ...
  | elt :: tr -> (
      try
        ...
        fprintf ppf "@[<v>@[%t%t@]%a%t@]" head_error ty_expect_explanation
          (trace false "is not compatible with type")
          tr (explain mis);
        if env <> Env.empty then warn_on_missing_defs env ppf head;
...
let report_unification_error ppf env tr
  ...
  wrap_printing_env env
    (fun () -> unification_error env tr txt1 ppf txt2 type_expected_explanation)
    ~error:true
  ...
  try
    ...
    | elt :: tr' ->
        ...
        let tr =
          ...
          @@ filter_trace keep_last tr'
        in
        if fst then trace fst txt ppf (elt @ tr) else trace fst txt ppf tr;
        print_labels := true
    | _ -> ()
  with exn ->
    print_labels := true;
  ...
  wrap_printing_env ~error:true env (fun () ->
      ...
      if tr2 = [] then fprintf ppf "@]"
        ...
        fprintf ppf "%a%t%t@]"
          ...
          tr2 (explain mis) Conflicts.print_explanations)
...
let report_ambiguous_type_error ppf env tp0 tpl txt1 txt2 txt3 =
  wrap_printing_env ~error:true env (fun () ->
      ...
      | [] -> assert false
      | [ tp ] ->
          fprintf ppf "@[%t@;<1 2>%a@ %t@;<1 2>%a@]" txt1 type_path_expansion
            (trees_of_type_path_expansion tp)
