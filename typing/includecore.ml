...
type label_mismatch = Type | Mutability of position
...
let report_record_mismatch first second decl ppf err =
  ...
  | Label_mismatch (l1, l2, err) ->
      pr
        "@[<hv>Fields do not match:@;\
         <1 2>%a@ is not compatible with:@;\
         <1 2>%a@ %a" Printtyp.label l1 Printtyp.label l2
        (report_label_mismatch first second)
        err
  | Label_names (n, name1, name2) ->
      pr "@[<hv>Fields number %i have different names, %s and %s.@]" n
        (Ident.name name1) (Ident.name name2)
  | Label_missing (ord, s) ->
      pr "@[<hv>The field %s is only present in %s %s.@]" (Ident.name s)
  ...
  | Constructor_mismatch (c1, c2, err) ->
      pr
        "@[<hv>Constructors do not match:@;\
         <1 2>%a@ is not compatible with:@;\
         <1 2>%a@ %a" Printtyp.constructor c1 Printtyp.constructor c2
        (report_constructor_mismatch first second decl)
        err
  | Constructor_names (n, name1, name2) ->
      pr "Constructors number %i have different names, %s and %s." n
        (Ident.name name1) (Ident.name name2)
  | Constructor_missing (ord, s) ->
      pr "The constructor %s is only present in %s %s." (Ident.name s)
        (choose ord first second) decl
  ...
  | Constructor_mismatch (id, ext1, ext2, err) ->
      pr
        "@[<hv>Constructors do not match:@;\
         <1 2>%a@ is not compatible with:@;\
         <1 2>%a@ %a@]"
        (Printtyp.extension_only_constructor id)
        ext1
        (Printtyp.extension_only_constructor id)
        ext2
        (report_constructor_mismatch first second decl)
...
let report_type_mismatch first second decl ppf err =
  ...
  else Format.fprintf ppf "@ %a" (report_type_mismatch0 first second decl) err
...
and compare_variants ~loc env params1 params2 n
  ...
  | cd1 :: rem1, cd2 :: rem2 ->
      if Ident.name cd1.cd_id <> Ident.name cd2.cd_id then
        ...
        with
        | Some r -> Some (Constructor_mismatch (cd1, cd2, r) : variant_mismatch)
        | None -> compare_variants ~loc env params1 params2 (n + 1) rem1 rem2)

and compare_labels env params1 params2 (ld1 : Types.label_declaration)
    (ld2 : Types.label_declaration) =
  if ld1.ld_mutable <> ld2.ld_mutable then
    let ord = if ld1.ld_mutable = Asttypes.Mutable then First else Second in
    Some (Mutability ord)
  else if Ctype.equal env true (ld1.ld_type :: params1) (ld2.ld_type :: params2)
  then None
...
and compare_records ~loc env params1 params2 n
  ...
  | ld1 :: rem1, ld2 :: rem2 ->
      if Ident.name ld1.ld_id <> Ident.name ld2.ld_id then
        Some (Label_names (n, ld1.ld_id, ld2.ld_id))
      else (
        ...
        | None ->
            compare_records ~loc env (ld1.ld_type :: params1)
              (ld2.ld_type :: params2) (n + 1) rem1 rem2)

let compare_records_with_representation ~loc env params1 params2 n labels1
    labels2 rep1 rep2 =
...
let type_declarations ?(equality = false) ~loc env ~mark name decl1 path decl2 =
  ...
  else if not (private_flags decl1 decl2) then Some Privacy
  else
    let err =
      match (decl1.type_manifest, decl2.type_manifest) with
      | _, None ->
          if Ctype.equal env true decl1.type_params decl2.type_params then None
      ...
      if err <> None then err
        ...
        let err =
          ...
          | Type_record (labels1, rep1), Type_record (labels2, rep2) ->
              Option.map
                (fun rec_err -> Record_mismatch rec_err)
                (compare_records_with_representation ~loc env decl1.type_params
                   decl2.type_params 1 labels1 labels2 rep1 rep2)
        ...
        if err <> None then err
          ...
          let err =
            if not abstr then None
            else
              match
                Type_immediacy.coerce decl1.type_immediate
                  ~as_:decl2.type_immediate
              with
              | Ok () -> None
          ...
          if err <> None then err
            ...
            in
            if not need_variance then None
            else
...
let extension_constructors ~loc env ~mark id ext1 ext2 =
  (if mark then
   let usage =
     if ext2.ext_private = Public then Env.Positive else Env.Privatize
   in
  ...
  then Some (Constructor_mismatch (id, ext1, ext2, Type))
    ...
    let r =
      compare_constructors ~loc env ext1.ext_type_params ext2.ext_type_params
        ext1.ext_ret_type ext2.ext_ret_type ext1.ext_args ext2.ext_args
    in
    match r with
