...
module Unification_trace = struct
  ...
  let map_elt f = function
    ...
    | ( Rec_occur (_, _)
      ...
      | Variant _ | Obj _ | Incompatible_fields _ ) as x ->
        x

  let map f = List.map (map_elt f)

  (* Convert desc to type_expr * type_expr *)
  let flatten_desc f x =
    match x.expanded with None -> f x.t x.t | Some expanded -> f x.t expanded
...
let proper_abbrevs path tl abbrev =
  if
    tl <> [] || !trace_gadt_instances || !Clflags.principal
    || is_object_type path
  then abbrev
  else simple_abbrevs
...
let can_generate_equations () =
  match !equations_generation with Forbidden -> false | _ -> true
...
let object_fields ty =
  match (repr ty).desc with Tobject (fields, _) -> fields | _ -> assert false
...
let rec has_dummy_method ty =
  match repr ty with
  | { desc = Tfield (m, _, _, ty2) } -> m = dummy_method || has_dummy_method ty2
  | _ -> false
...
let concrete_object ty =
  match (object_row ty).desc with Tvar _ -> false | _ -> true
...
let close_object ty =
  let rec close ty =
    ...
    | _ -> assert false
  in
  match (repr ty).desc with Tobject (ty, _) -> close ty | _ -> assert false
...
let row_variable ty =
  let rec find ty =
    ...
    | _ -> assert false
  in
  match (repr ty).desc with Tobject (fi, _) -> find fi | _ -> assert false
...
let rec lower_contravariant env var_level visited contra ty =
  ...
  if must_visit then (
    ...
    | Tconstr (path, tyl, _abbrev) ->
        let variance, maybe_expand =
          try
            ...
            (typ.type_variance, typ.type_kind = Type_abstract)
          with Not_found ->
            (* See testsuite/tests/typing-missing-cmi-2 for an example *)
            (List.map (fun _ -> Variance.unknown) tyl, false)
        in
        if List.for_all (( = ) Variance.null) variance then ()
        else
          let not_expanded () =
            List.iter2
              (fun v t ->
                if v = Variance.null then ()
                else if Variance.(mem May_weak v) then lower_rec true t
    ...
    | Tpackage (_, _, tyl) -> List.iter (lower_rec true) tyl
    | Tarrow (_, t1, t2, _) ->
        lower_rec true t1;
        lower_rec contra t2
                      ...
                      | Some (free_univars, false) ->
                          ...
                          then
                            ( more',
                              {
                                row_fields =
                                  List.filter not_reither row.row_fields;
                                row_more = more';
                                row_bound = ();
                                row_closed = false;
                                row_fixed = None;
...
let instance_declaration decl =
  For_copy.with_scope (fun scope ->
      {
        decl with
        type_params = List.map (copy scope) decl.type_params;
        ...
        Cty_signature
          {
            ...
            csig_inher =
              List.map
                (fun (p, tl) -> (p, List.map (copy scope) tl))
                sign.csig_inher;
          }
    | Cty_arrow (l, ty, cty) ->
...
let rec copy_sep cleanup_scope fixed free bound visited ty =
  ...
  if TypeSet.is_empty univars then (
    if ty.level <> generic_level then ty
      ...
      let t = newvar () in
      delayed_copy :=
        lazy (t.desc <- Tlink (copy cleanup_scope ty)) :: !delayed_copy;
      t)
  else
    try
...
let occur env ty0 ty =
  let allow_recursive =
    !Clflags.recursive_types || (!umode = Pattern && !allow_recursive_equation)
  in
  ...
  with exn -> (
    merge type_changed old;
    match exn with Occur -> raise (Trace.rec_occur ty0 ty) | _ -> raise exn)
...
let rec local_non_recursive_abbrev ~allow_rec strict visited env p ty =
  ...
  if not (List.memq ty visited) then
    ...
    | Tconstr (p', args, _abbrev) -> (
        ...
        if allow_rec && (not strict) && is_contractive env p' then ()
        else
...
let get_gadt_equations_level () =
  match !gadt_equations_level with None -> assert false | Some x -> x
    ...
    if TypeSet.mem ty !visited then ()
      ...
      | Tvariant r ->
          ...
          (if not (static_row r) then
           if is_fixed r then iterator (row_more r)
           else
             let m = r.row_more in
             match m.desc with
             | Tvar o ->
                 ...
                 let row =
                   ...
                   { r with row_fields = []; row_fixed; row_more = t }
                 in
    ...
    | n :: nl, _ -> (
        ...
        | ( _,
            ...
            | exception Nondep_cannot_erase _ ->
                if allow_absent then complete nl ntl2 else raise Exit)
        | ( _,
            {
              type_arity = 0;
              type_kind = Type_abstract;
          ...
          when allow_absent ->
            complete nl ntl2
        | _ -> raise Exit
        | exception Not_found when allow_absent -> complete nl ntl2)
  in
        ...
        | Tconstr (p1, [], _), Tconstr (p2, [], _)
          when Env.has_local_constraints !env
            ...
            try
              ...
              then unify env t1 (try_expand_once !env t2)
...
and unify2 env t1 t2 =
  ...
  if unify_eq t1' t2' then ()
    ...
    if unify_eq t1 t1' || not (unify_eq t2 t2') then unify3 env t1 t1' t2 t2'
      ...
      with Unify trace -> raise (Unify (Trace.swap trace))
...
and unify3 env t1 t1' t2 t2' =
  ...
  | _ -> (
      ...
      | Expression ->
          occur !env t1' t2';
          if is_self_type d1 (* PR#7711: do not abbreviate self type *) then
            link_type t1' t2'
      ...
      try
        ...
        | Tconstr (p1, tl1, _), Tconstr (p2, tl2, _) when Path.same p1 p2 ->
            if !umode = Expression || !equations_generation = Forbidden then
              ...
              set_mode_pattern ~generate:!equations_generation ~injective:false
                ~allow_recursive:!allow_recursive_equation (fun () ->
                  unify_list env tl1 tl2)
            else if
        ...
        | Tconstr (path, [], _), Tconstr (path', [], _)
          when is_instantiable !env path && is_instantiable !env path'
            ...
            let source, destination =
              if Path.scope path > Path.scope path' then (path, t2')
              else (path', t1')
            in
        ...
        | Tfield (f, kind, _, rem), Tnil | Tnil, Tfield (f, kind, _, rem) -> (
            ...
            | _ ->
                ...
                else raise (Unify Trace.[ Obj (Missing_field (Second, f)) ]))
        | Tnil, Tnil -> ()
        ...
        | Tpackage (p1, n1, tl1), Tpackage (p2, n2, tl2) -> (
            try
              unify_package !env (unify_list env) t1.level p1 n1 tl1 t2.level p2
                n2 tl2
            with Not_found ->
...
and make_rowvar level use1 rest1 use2 rest2 =
  let set_name ty name =
    match ty.desc with Tvar None -> set_type_desc ty (Tvar name) | _ -> ()
  in
  let name =
    ...
    let set_more row rest =
      ...
      | Some fixed ->
          ...
          if closed && not row.row_closed then
            raise
              Trace.(
                Unify [ Variant (Fixed_row (pos, Cannot_be_closed, fixed)) ])
          else if rest <> [] then
...
and unify_row_field env fixed1 fixed2 rm1 rm2 l f1 f2 =
  ...
  let if_not_fixed (pos, fixed) f =
    ...
    | Some fix ->
        let tr =
          Trace.[ Variant (Fixed_row (pos, Cannot_add_tags [ l ], fix)) ]
        in
        raise (Unify tr)
  in
  ...
  if f1 == f2 then ()
    ...
    | Reither (c1, tl1, m1, e1), Reither (c2, tl2, m2, e2) ->
        if e1 == e2 then ()
          ...
          either_fixed && (not (c1 || c2)) && List.length tl1 = List.length tl2
        then (
          (* PR#7496 *)
          ...
          let redo =
            ...
            | t1 :: tl ->
                ...
                !e1 <> None || !e2 <> None
          in
          if redo then unify_row_field env fixed1 fixed2 rm1 rm2 l f1 f2
          else
            let tl1 = List.map repr tl1 and tl2 = List.map repr tl2 in
...
let unify_gadt ~equations_level:lev ~allow_recursive (env : Env.t ref) ty1 ty2 =
  try
    ...
    set_mode_pattern
      ...
      ~injective:true ~allow_recursive
...
let normalize_subst subst =
  if
    List.exists
      (function
        | { desc = Tlink _ }, _ | _, { desc = Tlink _ } -> true | _ -> false)
      !subst
        ...
        Vars.iter
          (fun lab (_mut, _v, ty) ->
            ...
            with Unify trace ->
              raise
                (Failure
                   [ CM_Val_type_mismatch (lab, env, expand_trace env trace) ]))
          sign2.csig_vars
    | _ -> raise (Failure [])
  ...
  let error =
    List.fold_right
      (fun (lab, k, _) err ->
        let err =
          let k = field_kind_repr k in
          match k with Fvar _ -> err | _ -> CM_Hide_public lab :: err
        in
          ...
          List.map
            (fun ((l, f) as orig) ->
              ...
              | Rpresent (Some t) ->
                  let t', c = build_subtype env visited loops posi level' t in
                  let f =
                    if posi && level > 0 then
                      Reither (false, [ t' ], false, ref None)
  ...
  if t1 == t2 then cstrs
    ...
    with Not_found -> (
      ...
      | Tpackage (p1, nl1, tl1), Tpackage (p2, nl2, tl2) -> (
          try
            let ntl1 =
              complete_type_list env nl2 t1.level (Mty_ident p1) nl1 tl1
            and ntl2 =
      ...
      List.iter
        (function
          | trace0, t1, t2, pairs -> (
              ...
              with Unify trace ->
                raise
                  (Subtype (expand_trace env (List.rev trace0), List.tl trace))))
        (List.rev cstrs)
...
let rec arity ty =
  match (repr ty).desc with Tarrow (_, _t1, t2, _) -> 1 + arity t2 | _ -> 0
        ...
        ty'.desc <-
          ...
          | Tvariant row -> (
              ...
              with Not_found -> (
                ...
                (* Return a new copy *)
                let row =
                  copy_row (nondep_type_rec env ids) true row true more'
                in
                match row.row_name with
...
let rec nondep_class_type env ids = function
  ...
  | Cty_constr (p, tyl, cty) ->
      Cty_constr
        ( p,
          List.map (nondep_type_rec env ids) tyl,
          nondep_class_type env ids cty )
