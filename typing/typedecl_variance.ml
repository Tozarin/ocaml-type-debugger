...
let compute_variance_type env ~check (required, loc) decl tyl =
  ...
  if check_injectivity then
    List.iter
      (fun ty ->
        if Btype.is_Tvar ty || mem Inj (get_variance ty tvl) then ()
        else
          ...
          let rec check ty =
            ...
            if TypeSet.mem ty !visited then ()
              ...
              if mem Inj (get_variance ty tvl) then ()
              else
                match ty.desc with
                | Tvar _ -> raise Exit
                | Tconstr _ -> (
  ...
  if check then (
    ...
    List.iter2
      (fun ty (c, n, i) ->
        ...
        if
          (Btype.is_Tvar ty && ((co && not c) || (cn && not n)))
          || ((not ij) && i)
        then
          raise
            (Error
  ...
  then
    ...
    | Type_variant tll -> (
        if List.for_all (fun c -> c.Types.cd_res = None) tll then
          ...
          let mn =
            List.map (fun (_, ty) -> (Types.Cstr_tuple [ ty ], None)) mn
          in
          let tll =
...
let check_variance_extension env decl ext rloc =
  ...
  ignore
    (compute_variance_extension env ~check:true decl ext.Typedtree.ext_type rloc)
...
let property : (prop, req) Typedecl_properties.property =
  ...
  let compute env decl req = compute_decl env ~check:false decl req in
  let update_decl decl variance = { decl with type_variance = variance } in
  let check env id decl req =
    if is_hash id then () else check_decl env decl req
  in
