...
let rec abbreviate_class_type path params cty =
  match cty with
  | Cty_constr (_, _, _) | Cty_signature _ -> Cty_constr (path, params, cty)
  | Cty_arrow (l, ty, cty) ->
...
let enter_val cl_num vars inh lab mut virt ty class_env loc =
  ...
  let id, virt =
    ...
    | Ctype.Unify tr ->
        raise
          (Error
             (loc, val_env, Field_type_mismatch ("instance variable", lab, tr)))
    | Not_found -> (None, virt)
  in
  let ((id, _) as result) =
    match id with
    | Some id -> (id, class_env)
    | None ->
        enter_met_env Location.none lab
          (Val_ivar (mut, cl_num))
...
and class_field_aux self_loc cl_num self_type meths vars
    (class_env, fields, concr_meths, warn_vals, inher, local_meths, local_vals)
    cf =
  ...
  | Pcf_inherit (ovf, sparent, super) ->
      ...
      let class_env, inh_vars =
        Vars.fold
          (fun lab info (class_env, inh_vars) ->
            ...
            let id, class_env =
              enter_val cl_num vars true lab mut vr ty class_env sparent.pcl_loc
            in
      ...
      let inh_meths =
        Concr.fold
          (fun lab rem -> (lab, Ident.create_local lab) :: rem)
          cl_sig.csig_concr []
      in
      (* Super *)
      let class_env, super =
        ...
        | Some { txt = name } ->
            let _id, class_env =
              enter_met_env
                ~check:(fun s -> Warnings.Unused_ancestor s)
                sparent.pcl_loc name
                (Val_anc (inh_meths, cl_num))
  ...
  | Pcf_val (lab, mut, Cfk_virtual styp) ->
      ...
      let id, class_env' =
        enter_val cl_num vars false lab.txt mut Virtual ty class_env loc
      in
      ( class_env',
        lazy
          (mkcf
             (Tcf_val
                (lab, mut, id, Tcfk_virtual cty, met_env == class_env'.met_env)))
        :: fields,
        concr_meths,
        warn_vals,
        inher,
  ...
  | Pcf_val (lab, mut, Cfk_concrete (ovf, sexp)) ->
      ...
      if Concr.mem lab.txt warn_vals then (
        if ovf = Fresh then
          Location.prerr_warning lab.loc
            (Warnings.Instance_variable_override [ lab.txt ]))
      else if ovf = Override then
        raise
          (Error (loc, val_env, No_overriding ("instance variable", lab.txt)));
      if !Clflags.principal then Ctype.begin_def ();
      ...
      let id, class_env' =
        enter_val cl_num vars false lab.txt mut Concrete exp.exp_type class_env
          loc
      in
      ( class_env',
        lazy
          (mkcf
             (Tcf_val
                ( lab,
                  mut,
                  id,
                  Tcfk_concrete (ovf, exp),
                  met_env == class_env'.met_env )))
        :: fields,
        concr_meths,
        Concr.add lab.txt warn_vals,
        inher,
        local_meths,
  ...
  | Pcf_method (lab, priv, Cfk_virtual sty) ->
      ...
      ( class_env,
        lazy (mkcf (Tcf_method (lab, priv, Tcfk_virtual cty))) :: fields,
        concr_meths,
        warn_vals,
        inher,
        local_meths,
  ...
  | Pcf_method (lab, priv, Cfk_concrete (ovf, expr)) ->
      ...
      if Concr.mem lab.txt concr_meths then (
        if ovf = Fresh then
      ...
      (try
       ...
       with Ctype.Unify trace ->
         raise
           (Error (loc, val_env, Field_type_mismatch ("method", lab.txt, trace))));
      ...
      ( class_env,
        field :: fields,
        Concr.add lab.txt concr_meths,
        warn_vals,
        inher,
        Concr.add lab.txt local_meths,
        local_vals )
          ...
          (Ctype.raise_nongen_level ();
           let meth_type =
             mk_expected
               (Ctype.newty
                  (Tarrow
                     (Nolabel, self_type, Ctype.instance Predef.type_unit, Cok)))
           in
           vars := vars_local;
...
and class_structure cl_num final val_env met_env loc
  ...
  let ty = if final then Ctype.newobj (Ctype.newvar ()) else self_type in
  (try Ctype.unify val_env public_self ty
   with Ctype.Unify _ ->
     raise (Error (spat.ppat_loc, val_env, Pattern_type_clash public_self)));
  let get_methods ty =
    fst
      (Ctype.flatten_fields
         (Ctype.object_fields (Ctype.expand_head val_env ty)))
  in
  if final then
    ...
    List.iter
      (fun (lab, kind, ty) ->
        let k =
          if Btype.field_kind_repr kind = Fpresent then Public else Private
        in
        try Ctype.unify val_env ty (Ctype.filter_method val_env lab k self_type)
        with _ -> assert false)
  ...
  let _, fields, concr_meths, _, inher, _local_meths, _local_vals =
    Builtin_attributes.warning_scope [] (fun () ->
        List.fold_left
          ...
          (class_env, [], Concr.empty, Concr.empty, [], Concr.empty, Concr.empty)
          str)
  in
  ...
  let priv_meths =
    List.filter
      (fun (_, kind, _) -> Btype.field_kind_repr kind <> Fpresent)
      methods
  in
  (* ensure that inherited methods are listed too *)
  List.iter
    (fun (met, _kind, _ty) ->
      if Meths.mem met !meths then ()
      else ignore (Ctype.filter_self_method val_env met Private meths self_type))
    methods;
  if final then (
  ...
  (let ms = !meths in
   ...
   meths := Meths.map (fun (id, ty) -> (id, Ctype.generic_instance ty)) ms;
  ...
  let sign =
    if final then sign
    else { sign with Types.csig_self = Ctype.expand_head val_env public_self }
  in
  ( {
      cstr_self = pat;
      cstr_fields = fields;
...
and class_expr_aux cl_num val_env met_env scl =
  ...
  | Pcl_constr (lid, styl) ->
      ...
      let tyl =
        List.map (fun sty -> transl_simple_type val_env false sty) styl
      in
            ...
            ( id,
              {
                exp_desc =
                  Texp_ident
                    (path, mknoloc (Longident.Lident (Ident.name id)), vd);
              ...
              if ignore_labels then
                ...
                | (l', sarg) :: remaining_sargs ->
                    ...
                    then (sargs, eliminate_optional_arg ())
                      ...
                      raise
                        (Error (sarg.pexp_loc, val_env, Apply_wrong_label l'))
              else
                ...
                | None ->
                    ( sargs,
                      if Btype.is_optional l && List.mem_assoc Nolabel sargs
  ...
  | Pcl_let (rec_flag, sdefs, scl') ->
      let defs, val_env =
        Typecore.type_let In_class_def val_env rec_flag sdefs
      in
      let vals, met_env =
        List.fold_right
          (fun (id, _id_loc, _typ) (vals, met_env) ->
            ...
            ((id', expr) :: vals, Env.add_value id' desc met_env))
          (let_bound_idents_full defs)
          ([], met_env)
      in
      let cl = class_expr cl_num val_env met_env scl' in
      let () =
        if rec_flag = Recursive then check_recursive_bindings val_env defs
      in
      rc
        {
...
let initial_env define_class approx (res, env)
    (cl, id, ty_id, obj_id, cl_id, uid) =
...
let class_infos define_class kind
  ...
  (if cl.pci_virt = Concrete then
   ...
   if mets <> [] || vals <> [] then
     raise
       (Error (cl.pci_loc, env, Virtual_class (define_class, false, mets, vals))));
...
let final_decl env define_class
  ...
  | Some reason ->
      let printer =
        if define_class then function
          | ppf -> Printtyp.class_declaration id ppf clty
...
and unify_parents_struct env ty st =
  List.iter
    (function
      | { cf_desc = Tcf_inherit (_, cl, _, _, _) } -> unify_parents env ty cl
      | _ -> ())
    st.cstr_fields
...
let report_error env ppf = function
  ...
  | Field_type_mismatch (k, m, trace) ->
      Printtyp.report_unification_error ppf env trace
        (function ppf -> fprintf ppf "The %s %s@ has type" k m)
  ...
  | Apply_wrong_label l ->
      let mark_label = function
        | Nolabel -> "out label"
        | l -> sprintf " label %s" (Btype.prefixed_label_name l)
      in
  ...
  | Abbrev_type_clash (abbrev, actual, expected) ->
      ...
      fprintf ppf
        "@[The abbreviation@ %a@ expands to type@ %a@ but is used with type@ \
         %a@]"
        !Oprint.out_type
        (Printtyp.tree_of_typexp false abbrev)
        !Oprint.out_type
        (Printtyp.tree_of_typexp false actual)
        !Oprint.out_type
        (Printtyp.tree_of_typexp false expected)
  | Constructor_type_mismatch (c, trace) ->
      Printtyp.report_unification_error ppf env trace
        (function ppf -> fprintf ppf "The expression \"new %s\" has type" c)
        (function ppf -> fprintf ppf "but is used with type")
  | Virtual_class (cl, imm, mets, vals) ->
      let print_mets ppf mets =
        List.iter (function met -> fprintf ppf "@ %s" met) mets
      in
      let missings =
  ...
  | Bad_parameters (id, params, cstrs) ->
      ...
      fprintf ppf
        ...
        Printtyp.ident id !Oprint.out_type
        (Printtyp.tree_of_typexp false params)
        !Oprint.out_type
        (Printtyp.tree_of_typexp false cstrs)
  | Class_match_failure error -> Includeclass.report_error ppf error
  | Unbound_val lab -> fprintf ppf "Unbound instance variable %s" lab
  | Unbound_type_var (printer, reason) ->
      let print_common ppf kind ty0 real lab ty =
        ...
        fprintf ppf "The %s %s@ has type@;<1 2>%a@ where@ %a@ is unbound" kind
          lab !Oprint.out_type
          (Printtyp.tree_of_typexp false ty)
          !Oprint.out_type
          (Printtyp.tree_of_typexp false ty0)
      in
  ...
  | Non_collapsable_conjunction (id, clty, trace) ->
      fprintf ppf
        "@[The type of this class,@ %a,@ contains non-collapsible conjunctive \
         types in constraints.@ %t@]" (Printtyp.class_declaration id) clty
        (fun ppf ->
          Printtyp.report_unification_error ppf env trace
            (fun ppf -> fprintf ppf "Type")
            (fun ppf -> fprintf ppf "is not compatible with type"))
  ...
  | Mutability_mismatch (_lab, mut) ->
      let mut1, mut2 =
        ...
        else ("immutable", "mutable")
      in
      fprintf ppf
        "@[The instance variable is %s;@ it cannot be redefined as %s@]" mut1
        mut2
  | No_overriding (_, "") ->
