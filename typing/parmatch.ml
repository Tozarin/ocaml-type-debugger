...
let extra_pat =
  make_pat (Tpat_var (Ident.create_local "+", mknoloc "+")) Ctype.none Env.empty
  ...
  let coherent_heads hp1 hp2 =
    ...
    | Constant c1, Constant c2 -> (
        ...
        | ( ( Const_char _ | Const_int _ | Const_int32 _ | Const_int64 _
            | Const_nativeint _ | Const_float _ | Const_string _ ),
            _ ) ->
            false)
...
let const_compare x y =
  ...
  | ( ( Const_int _ | Const_char _
      | Const_string (_, _, _)
      | Const_float _ | Const_int32 _ | Const_int64 _ | Const_nativeint _ ),
      _ ) ->
      Stdlib.compare x y
...
let get_constructor_type_path ty tenv =
  ...
  match ty.desc with Tconstr (path, _, _) -> path | _ -> assert false
...
let simple_match d h =
  ...
  | Tuple len1, Tuple len2 | Array len1, Array len2 -> len1 = len2
  | _, Any -> true
  | _, _ -> false
...
let simple_match_args discr head args =
  ...
  | Constant _ -> []
  | Construct _ | Variant _ | Tuple _ | Array _ | Lazy -> args
  ...
  | Any -> (
      ...
      | Array len | Tuple len -> Patterns.omegas len
    ...
    | ((head, _), _) :: rows -> (
        ...
        | Record lbls ->
            ...
            let fields =
              List.fold_right
                (fun lbl r ->
                  if List.exists (fun l -> l.lbl_pos = lbl.lbl_pos) r then r
                  else lbl :: r)
                lbls (record_arg acc)
            in
...
let build_specialized_submatrix ~extend_row discr pss =
  let rec filter_rec = function
    | ((head, args), ps) :: pss ->
        if simple_match discr head then
          extend_row (simple_match_args discr head args) ps :: filter_rec pss
        else filter_rec pss
    | _ -> []
  in
  filter_rec pss
...
let build_specialized_submatrices ~extend_row discr rows =
  ...
  *)
  let rec insert_constr head args r = function
    ...
    | ((q0, rs) as bd) :: env ->
        if simple_match q0 head then extend_group q0 head args r rs :: env
...
let should_extend ext env =
  ...
  | Some ext -> (
      ...
      | (p, _) :: _ -> (
          ...
          | Construct
              { cstr_tag = Cstr_constant _ | Cstr_block _ | Cstr_unboxed } ->
              let path = get_constructor_type_path p.pat_type p.pat_env in
              Path.same path ext
          | Construct { cstr_tag = Cstr_extension _ } -> false
          | Constant _ | Tuple _ | Variant _ | Record _ | Array _ | Lazy ->
              false
          | Any -> assert false))
...
let pat_of_constr ex_pat cstr =
  {
    ...
    pat_desc =
      Tpat_construct
        (mknoloc (Longident.Lident cstr.cstr_name), cstr, omegas cstr.cstr_arity);
  }
...
let build_other ext env =
  ...
  | (d, _) :: _ -> (
      ...
      | Variant { cstr_row; type_row } -> (
          ...
          | [] ->
              let tag =
                if Btype.row_fixed row then some_private_tag
                else
                  let rec mktag tag =
      ...
      | Constant (Const_char _) ->
          ...
          let rec find_other i imax =
            if i > imax then raise Not_found
              ...
              if List.mem ci all_chars then find_other (i + 1) imax
      ...
      | Constant (Const_float _) ->
          build_other_constant
            (function
              | Constant (Const_float f) -> float_of_string f
              | _ -> assert false)
      ...
      | Array _ ->
          let all_lengths =
            List.map
              (fun (p, _) ->
                match p.pat_desc with Array len -> len | _ -> assert false)
              env
          in
          let rec try_arrays l =
            if List.mem l all_lengths then try_arrays (l + 1)
            else make_pat (Tpat_array (omegas l)) d.pat_type d.pat_env
          in
          try_arrays 0
      | _ -> Patterns.omega)
  ...
  | _ -> (
      ...
      | q :: qs -> (
          ...
          | #Patterns.Simple.view as view ->
              ...
              if not (all_coherent (hq :: first_column pss)) then false
              else
                             ...
                             if is_absent_pat p then []
                               ...
                               let witnesses =
                                 list_satisfying_vectors pss
                                   (simple_match_args p Patterns.Head.omega []
                                   @ qs)
                               in
  ...
  if not (all_coherent (first_column pss)) then
    ...
    | { default; constrs } ->
        let try_non_omega (p, pss) =
          if is_absent_pat p then Seq.empty
          else
            let sub_witnesses =
              exhaust ext pss
        ...
        | { default; constrs } ->
            ...
            if full_match (tdefs = None) constrs then try_non_omega constrs
              ...
              | (d, _) :: _, Some env -> (
                  ...
                  | Variant { type_row; _ } ->
                      let row = type_row () in
                      if Btype.row_fixed row || pressure_variants None default
                      then ()
  ...
  | q :: rem -> (
      ...
      | #Patterns.Simple.view as view ->
          ...
          (* The handling of incoherent matrices is kept in line with
             [satisfiable] *)
          if not (all_coherent (hq :: first_column pss)) then Unused
          else
            let q0 = discr_pat q pss in
            every_satisfiables
...
let pressure_variants tdefs patl =
  ignore
    (pressure_variants (Some tdefs) (List.map (fun p -> [ p; omega ]) patl))
...
let rec initial_only_guarded = function
  | [] -> []
  | { c_guard = None; _ } :: rem -> initial_only_guarded rem
  | { c_lhs = pat; _ } :: rem -> [ pat ] :: initial_only_guarded rem
...
let do_check_partial ~pred loc casel pss =
  ...
  | ps :: _ -> (
      let counter_examples =
        exhaust None pss (List.length ps) |> Seq.filter_map (typecheck ~pred)
      in
...
let check_unused pred casel =
  if
    Warnings.is_active Warnings.Redundant_case
                   ...
                   List.iter
                     (fun p ->
                       Location.prerr_warning p.pat_loc
                         Warnings.Redundant_subpat)
                     ps
               | Used -> ()
           with Empty | Not_found -> assert false);

          if c_guard <> None then do_rec pref rem
          else do_rec ([ q ] :: pref) rem
    in
    ...
    if is_active warn0 then
      let check_case ns case =
        ...
        | { c_lhs = p; c_guard = Some g; _ } ->
            ...
            (if not (Ident.Set.is_empty all) then
             match pattern_stable_vars ns p with
             | All -> ()
             | Vars stable ->
                 let ambiguous = Ident.Set.diff all stable in
                 if not (Ident.Set.is_empty ambiguous) then
                   let pps =
                     Ident.Set.elements ambiguous |> List.map Ident.name
                   in
