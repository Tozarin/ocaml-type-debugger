...
let value_descriptions ~loc env ~mark cxt subst id vd1 vd2 =
  ...
  try Includecore.value_descriptions ~loc env (Ident.name id) vd1 vd2
...
let type_declarations ~loc env ~mark ?old_env:_ cxt subst id decl1 decl2 =
  ...
  match
    Includecore.type_declarations ~loc env ~mark (Ident.name id) decl1
      (Path.Pident id) decl2
  with
  | None -> ()
  | Some err ->
...
let class_type_declarations ~loc ~old_env:_ env cxt subst id decl1 decl2 =
  ...
  | reason ->
      raise
        (Error
           [ (cxt, env, Class_type_declarations (id, decl1, decl2, reason)) ])
...
let class_declarations ~old_env:_ env cxt subst id decl1 decl2 =
  ...
  | reason ->
      raise
        (Error [ (cxt, env, Class_declarations (id, decl1, decl2, reason)) ])
...
let item_ident_name = function
  ...
  | Sig_typext (id, d, _, _) ->
      let kind =
        if Path.same d.ext_type_path Predef.path_exn then
          Field_exception (Ident.name id)
...
let rec modtypes ~loc env ~mark cxt subst mty1 mty2 =
  ...
  | Dont_match ->
      raise
        (Error
           [
             (cxt, env, Module_types (mty1, Subst.modtype Make_local subst mty2));
           ])
...
and try_modtypes ~loc env ~mark cxt subst mty1 mty2 =
  ...
  | Mty_alias p1, Mty_alias p2 ->
      ...
      if not (equal_module_paths env p1 subst p2) then raise Dont_match;
      Tcoerce_none
  | Mty_alias p1, _ ->
      let p1 =
        try Env.normalize_module_path (Some Location.none) env p1
        with Env.Error (Env.Missing_module (_, _, path)) ->
          raise (Error [ (cxt, env, Unbound_module_path path) ])
      in
      let mty1 = expand_module_alias env cxt p1 in
      strengthened_modtypes ~loc ~aliasable:true env ~mark cxt subst mty1 p1
        mty2
  ...
  | ( Mty_functor ((Named (param1, arg1) as arg), res1),
      ...
      let cc_arg =
        modtypes ~loc env ~mark:(negate_mark mark) (Arg arg :: cxt)
      ...
      let env, subst =
        ...
        | None, Some p2 -> (Env.add_module p2 Mp_present arg2' env, subst)
        | Some p1, None -> (Env.add_module p1 Mp_present arg2' env, subst)
        | None, None -> (env, subst)
      in
...
and signatures ~loc env ~mark cxt subst sig1 sig2 =
  ...
  let rec build_component_table pos tbl = function
    ...
    | (( Sig_value (_, _, Hidden)
       ...
       | Sig_class_type (_, _, _, Hidden) ) as item)
      :: rem ->
    ...
    | item :: rem ->
        ...
        let pos, nextpos =
          if is_runtime_component item then (pos, pos + 1) else (-1, pos)
        in
        build_component_table nextpos
          (FieldMap.add name (id, item, pos) tbl)
          rem
  in
  let len1, comps1 = build_component_table 0 FieldMap.empty sig1 in
  let len2 =
    List.fold_left
...
and signature_components ~loc old_env ~mark env cxt subst paired =
  ...
  | (Sig_class (id1, decl1, _, _), Sig_class (_id2, decl2, _, _), pos) :: rem ->
      ...
      (pos, Tcoerce_none) :: comps_rec rem
  | (Sig_class_type (id1, info1, _, _), Sig_class_type (_id2, info2, _, _), _pos)
    :: rem ->
...
and module_declarations ~loc env ~mark cxt subst id1 md1 md2 =
  Builtin_attributes.check_alerts_inclusion ~def:md1.md_loc ~use:md2.md_loc loc
    md1.md_attributes md2.md_attributes (Ident.name id1);
...
and modtype_infos ~loc env ~mark cxt subst id info1 info2 =
  ...
  try
    ...
    | None, Some mty2 ->
        check_modtype_equiv ~loc env ~mark cxt' (Mty_ident (Path.Pident id))
          mty2
  with Error reasons ->
...
let check_modtype_inclusion ~loc env mty1 path1 mty2 =
  ...
  ignore
    (strengthened_modtypes ~loc ~aliasable env ~mark:Mark_both [] Subst.identity
       mty1 path1 mty2)

let () =
  Env.check_functor_application :=
    fun ~errors ~loc env mty1 path1 mty2 path2 ->
      try check_modtype_inclusion ~loc env mty1 path1 mty2
      with Error errs ->
        if errors then raise (Apply_error (loc, path1, path2, errs))
...
let strengthened_module_decl ~loc ~aliasable env ~mark md1 path1 md2 =
  strengthened_module_decl ~loc ~aliasable env ~mark [] Subst.identity md1 path1
    md2
...
module Illegal_permutation = struct
  ...
  type coerce_pos = Item of int | InArg | InBody
  ...
  and not_fixpoint path pos = function
    | [] -> None
    | (n, _) :: q ->
        if n = pos then not_fixpoint path (pos + 1) q
  ...
  let rec runtime_item k = function
    | [] -> raise Not_found
    | item :: q ->
        if not (is_runtime_component item) then runtime_item k q
        else if k = 0 then item
  ...
  let rec find env ctx path mt =
    ...
    | Mty_functor (arg, mt), InBody :: q -> find env (Body arg :: ctx) q mt
    | _ -> raise Not_found
    ...
    try
      ...
      Format.fprintf ppf
        "@[<hv 2>Illegal permutation of runtime components in a module type.@ \
         @[For example,@ %a@[the %a@ and the %a are not in the same order@ in \
...
let rec context ppf = function
  ...
  | Modtype id :: rem ->
      fprintf ppf "@[<2>module type %a =@ %a@]" Printtyp.ident id context_mty
        rem
  | Body x :: rem ->
...
and args ppf = function
  ...
  | Arg x :: rem -> fprintf ppf "(%s :@ %a) : ..." (argname x) context_mty rem
...
let include_err env ppf = function
  | Missing_field (id, loc, kind) ->
      fprintf ppf "The %s `%a' is required but not provided" kind Printtyp.ident
        id;
  ...
  | Value_descriptions (id, d1, d2) ->
      fprintf ppf
        ...
        !Oprint.out_sig_item
        (Printtyp.tree_of_value_description id d1)
        !Oprint.out_sig_item
        (Printtyp.tree_of_value_description id d2);
  ...
  | Type_declarations (id, d1, d2, err) ->
      fprintf ppf "@[<v>@[<hv>%s:@;<1 2>%a@ %s@;<1 2>%a@]%a%a@]"
        ...
        err show_locs (d1.type_loc, d2.type_loc)
  ...
  | Class_type_declarations (id, d1, d2, reason) ->
      fprintf ppf
        "@[<hv 2>Class type declarations do not match:@ %a@;\
         <1 -2>does not match@ %a@]@ %a" !Oprint.out_sig_item
  ...
  | Class_declarations (id, d1, d2, reason) ->
      fprintf ppf
        "@[<hv 2>Class declarations do not match:@ %a@;\
         <1 -2>does not match@ %a@]@ %a" !Oprint.out_sig_item
        (Printtyp.tree_of_class_declaration id d1 Trec_first)
        !Oprint.out_sig_item
        (Printtyp.tree_of_class_declaration id d2 Trec_first)
        Includeclass.report_error reason
