...
let newty2 level desc =
  incr new_id;
  { desc; level; scope = lowest_level; id = !new_id }
...
let rec rev_concat l ll =
  match ll with [] -> l | l' :: ll -> rev_concat (l' @ l) ll
...
let is_fixed row = match row.row_fixed with None -> false | Some _ -> true
...
let row_of_type t =
  ...
  | Tobject (t, _) ->
      let rec get_row t =
        let t = repr t in
        match t.desc with Tfield (_, _, _, t) -> get_row t | _ -> t
      in
      get_row t
...
let iter_row f row = fold_row (fun () v -> f v) () row
...
let iter_type_expr f ty = fold_type_expr (fun () v -> f v) () ty
...
let type_iterators =
  ...
  and it_module_type it = function
    | Mty_ident p | Mty_alias p -> it.it_path p
...
module For_copy : sig
  ...
  let dup_kind copy_scope r =
    ...
    if not (List.memq r copy_scope.new_kinds) then (
...
let rec extract_label_aux hd l = function
  | [] -> None
  | ((l', t) as p) :: ls ->
      if label_name l' = l then Some (l', t, hd <> [], List.rev_append hd ls)
...
let set_level ty level =
  if level <> ty.level then (
    if ty.id <= !last_snapshot then log_change (Clevel (ty, ty.level));
    ty.level <- level)

let set_scope ty scope =
  if scope <> ty.scope then (
