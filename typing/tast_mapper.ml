...
let pat : type k. mapper -> k general_pattern -> k general_pattern =
 fun sub x ->
  ...
  let pat_desc : k pattern_desc =
    ...
    | Tpat_any | Tpat_var _ | Tpat_constant _ -> x.pat_desc
...
let expr sub x =
  ...
  let exp_desc =
    ...
    | Texp_match (exp, cases, p) ->
        Texp_match (sub.expr sub exp, List.map (sub.case sub) cases, p)
    | Texp_try (exp, cases) ->
    ...
    | Texp_variant (l, expo) -> Texp_variant (l, Option.map (sub.expr sub) expo)
    ...
    | Texp_send (exp, meth, expo) ->
        Texp_send (sub.expr sub exp, meth, Option.map (sub.expr sub) expo)
    | (Texp_new _ | Texp_instvar _) as d -> d
    | Texp_setinstvar (path1, path2, id, exp) ->
        Texp_setinstvar (path1, path2, id, sub.expr sub exp)
    | Texp_override (path, list) ->
        Texp_override (path, List.map (tuple3 id id (sub.expr sub)) list)
    | Texp_letmodule (id, s, pres, mexpr, exp) ->
        Texp_letmodule (id, s, pres, sub.module_expr sub mexpr, sub.expr sub exp)
...
let binding_op sub x = { x with bop_exp = sub.expr sub x.bop_exp }
...
let open_description sub od = { od with open_env = sub.env sub od.open_env }
...
let row_field sub x =
  let rf_desc =
    ...
    | Ttag (label, b, list) -> Ttag (label, b, List.map (sub.typ sub) list)
...
let object_field sub x =
  let of_desc =
    ...
    | OTtag (label, ct) -> OTtag (label, sub.typ sub ct)
...
let class_field sub x =
  let cf_desc =
    ...
    | Tcf_val (s, mf, id, k, b) -> Tcf_val (s, mf, id, class_field_kind sub k, b)
    | Tcf_method (s, priv, k) -> Tcf_method (s, priv, class_field_kind sub k)
