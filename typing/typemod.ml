...
let initial_env ~loc ~safe_string ~initially_opened_module
  ...
  let env =
    if safe_string then Env.initial_safe_string else Env.initial_unsafe_string
  in
  ...
  let add_units env units =
    String.Set.fold
      ...
      units env
  in
  let units = List.map Env.persistent_structures_of_dir (Load_path.get ()) in
  let env, units =
      ...
      | Sig_module (id, _, mty, Trec_first, _) :: rem ->
          ...
          (try check_recmod_typedecls (Lazy.force env) ((id, mty) :: id_mty_l)
           with Typedecl.Error (_, err) ->
             raise
               (Error
                  (loc, Lazy.force env, Badly_formed_signature (context, err))));
...
let merge_constraint initial_env remove_aliases loc sg constr =
  ...
  let rec merge sig_env sg namelist row_id =
    ...
    | [], _, _ -> raise (Error (loc, sig_env, With_no_component lid.txt))
    | ( Sig_type (id, decl, rs, priv) :: rem,
        [ s ],
      ...
      when Ident.name id = s && Typedecl.is_fixed_type sdecl ->
        ...
        in
        let tdecl =
          Typedecl.transl_with_constraint id (Some (Pident id_row)) ~sig_env
            ~sig_decl:decl ~outer_env:initial_env sdecl
        in
        ...
        ( (Pident id, lid, Twith_type tdecl),
          ...
          :: rem )
    | ( Sig_type (id, sig_decl, rs, priv) :: rem,
        [ s ],
        ((Pwith_type (_, sdecl) | Pwith_typesubst (_, sdecl)) as constr) )
      when Ident.name id = s -> (
        let tdecl =
          Typedecl.transl_with_constraint id None ~sig_env ~sig_decl
            ~outer_env:initial_env sdecl
        in
    ...
    | Sig_module (id, pres, md, rs, priv) :: rem, [ s ], Pwith_module (_, lid')
      when Ident.name id = s ->
        ...
        ignore
          (Includemod.modtypes ~mark:Mark_both ~loc sig_env newmd.md_type
             md.md_type);
    ...
    | Sig_module (id, _, md, rs, _) :: rem, [ s ], Pwith_modsubst (_, lid')
      when Ident.name id = s ->
        ...
        ignore
          (Includemod.strengthened_module_decl ~loc ~mark:Mark_both ~aliasable
             sig_env md' path md);
        real_ids := [ Pident id ];
    ...
    | item :: rem, _, _ ->
        let cstr, items = merge sig_env rem namelist row_id in
        (cstr, item :: items)
  ...
  try
    ...
    (if destructive_substitution then
     ...
     | last :: rest -> (
         ...
         | [] -> ()
         | _ :: _ ->
             check_usage_of_path_of_substituted_item rest initial_env sg ~loc
               ~lid));
    let sg =
      match tcstr with
          ...
          let how_to_extend_subst =
            ...
            | None ->
                ...
                if params_are_constrained params then
                  raise
                    (Error
                       (loc, initial_env, With_cannot_remove_constrained_type));
                fun s path -> Subst.add_type_function path ~params ~body s
          in
          let sub =
            List.fold_left how_to_extend_subst Subst.identity !real_ids
          in
...
and approx_sig env ssg =
  ...
  | item :: srem -> (
      ...
      | Psig_module { pmd_name = { txt = None; _ }; _ } -> approx_sig env srem
      | Psig_module pmd ->
          let scope = Ctype.create_scope () in
...
module Signature_names : sig
  ...
  type bound_info = [ `Exported | `Shadowable of Ident.t * Location.t ]
  type info = [ `From_open | `Substituted_away of Subst.t | bound_info ]
  type hide_reason = From_open | Shadowed_by of Ident.t * Location.t
  ...
  type t = { bound : names; to_be_removed : to_be_removed }
  ...
  let check_value ?info t loc id =
    let info = match info with Some i -> i | None -> `Shadowable (id, loc) in
  ...
  let check_sig_item ?info names loc component =
    let info id loc =
      match info with None -> `Shadowable (id, loc) | Some i -> i
    in
    match component with
    | Sig_type (id, _, _, _) -> check_type names loc id ~info:(info id loc)
    | Sig_module (id, _, _, _, _) ->
        check_module names loc id ~info:(info id loc)
    | Sig_modtype (id, _, _) -> check_modtype names loc id ~info:(info id loc)
    | Sig_typext (id, _, _, _) -> check_typext names loc id ~info:(info id loc)
    | Sig_value (id, _, _) -> check_value names loc id ~info:(info id loc)
    | Sig_class (id, _, _, _) -> check_class names loc id ~info:(info id loc)
  ...
  *)
  ...
  let simplify env t sg =
    ...
    let aux component sg =
      ...
      in
      if Ident.Map.mem user_id to_remove.hide then sg
      else
        let component =
          if to_remove.subst == Subst.identity then component
          else Subst.signature_item Keep to_remove.subst component
        in
        let component =
...
let has_remove_aliases_attribute attr =
  let remove_aliases =
    Attr_helper.get_no_payload_attribute
      [ "remove_aliases"; "ocaml.remove_aliases" ]
      attr
  in
...
and transl_modtype_aux env smty =
  ...
  | Pmty_ident lid ->
      ...
      mkmty
        (Tmty_ident (path, lid))
        (Mty_ident path) env loc smty.pmty_attributes
  | Pmty_alias lid ->
      let path = transl_module_alias loc env lid.txt in
      mkmty
        (Tmty_alias (path, lid))
        (Mty_alias path) env loc smty.pmty_attributes
  | Pmty_signature ssg ->
            ...
            let id, newenv =
              ...
              | Some name ->
                  ...
                  let id, newenv =
                    ...
                    in
                    Env.enter_module_declaration ~scope ~arg:true name
                      Mp_present arg_md env
                  in
                  (Some id, newenv)
            in
  ...
  let rec transl_sig env sg =
    ...
    | item :: srem -> (
        ...
        | Psig_type (rec_flag, sdecls) ->
            ...
            let sg =
              map_rec_type_with_row_types ~rec_flag
                ...
                decls rem
            in
            (mksig (Tsig_type (rec_flag, decls)) env loc :: trem, sg, final_env)
        | Psig_typesubst sdecls ->
            ...
            List.iter
              (fun td ->
                ...
                let info =
                  let subst =
                    Subst.add_type_function (Pident td.typ_id) ~params
            ...
            let trem, rem, final_env = transl_sig newenv srem in
            let sg = rem in
            (mksig (Tsig_typesubst decls) env loc :: trem, sg, final_env)
        | Psig_typext styext ->
            let tyext, newenv =
        ...
        | Psig_modsubst pms ->
            let scope = Ctype.create_scope () in
            let path, md =
              Env.lookup_module ~loc:pms.pms_manifest.loc pms.pms_manifest.txt
                env
            in
            let aliasable = not (Env.is_functor_arg path env) in
            let md =
              if not aliasable then md
              else
                {
            ...
            ( mksig
              ...
              final_env )
        | Psig_recmodule sdecls ->
            let tdecls, newenv = transl_recmodule_modtypes env sdecls in
            let decls =
              List.filter_map
                (fun (md, uid) ->
                  match md.md_id with
        ...
        | Psig_class cl ->
            ...
            let typedtree =
              mksig
                (Tsig_class
                   (List.map (fun decr -> decr.Typeclass.cls_info) classes))
                env loc
              :: trem
            in
                ...
                (fun rs decl ->
                  ...
                  [
                    Sig_class_type
                      (decl.clsty_ty_id, decl.clsty_ty_decl, rs, Exported);
                    Sig_type
                      (decl.clsty_obj_id, decl.clsty_obj_abbr, rs, Exported);
                    Sig_type
                      (decl.clsty_typesharp_id, decl.clsty_abbr, rs, Exported);
                  ])
                classes [ rem ]
              |> List.flatten
            in
...
and transl_recmodule_modtypes env sdecls =
    ...
    List.fold_left
      (fun env (id, _, md, _) ->
        Option.fold ~none:env
          ~some:(fun id ->
            Env.add_module_declaration ~check:true ~arg:true id Mp_present md
              env)
          id)
      env curr
  in
  ...
  let ids =
    List.map
      (fun x -> Option.map (Ident.create_scoped ~scope) x.pmd_name.txt)
      sdecls
  in
  let approx_env =
    List.fold_left
...
let check_recmodule_inclusion env bindings =
  ...
  let rec check_incl first_time n env s =
    ...
    if n > 0 then
      ...
      let check_inclusion (id, name, mty_decl, modl, mty_actual, attrs, loc, uid)
          =
...
let package_subtype env p1 nl1 tl1 p2 nl2 tl2 =
  let mkmty p nl tl =
    let ntl =
      List.filter
        (fun (_n, t) -> Ctype.free_variables t = [])
        (List.combine nl tl)
    in
...
let wrap_constraint env mark arg mty explicit =
  ...
  let coercion =
    try Includemod.modtypes ~loc:arg.mod_loc env ~mark arg.mod_type mty
    with Includemod.Error msg ->
...
and type_module_aux ~alias sttn funct_body anchor env smod =
  ...
  | Pmod_apply (sfunct, sarg) -> (
      ...
      let funct =
        type_module (sttn && path <> None) funct_body None env sfunct
      in
      ...
      | Mty_functor (Named (param, mty_param), mty_res) as mty_functor ->
          ...
          let mty_appl =
            ...
            | None ->
                ...
                in
                (match
                   Includemod.modtypes ~mark:Mark_neither ~loc:smod.pmod_loc env
                     mty_res nondep_mty
                 with
                | Tcoerce_none -> ()
                | _ ->
      ...
      let mty =
        ...
        | { desc = Tpackage (p, nl, tl) } ->
            if List.exists (fun t -> Ctype.free_variables t <> []) tl then
              raise
                (Error
                   (smod.pmod_loc, env, Incomplete_packed_module exp.exp_type));
            if
              !Clflags.principal
              && not
                   (Typecore.generalizable (Btype.generic_level - 1)
                      exp.exp_type)
            then
...
and type_structure ?(toplevel = false) funct_body anchor env sstr =
  ...
  let type_str_item env { pstr_loc = loc; pstr_desc = desc } =
    ...
    | Pstr_value (rec_flag, sdefs) ->
        let defs, newenv = Typecore.type_binding env rec_flag sdefs in
        let () =
          ...
          List.fold_left
            (fun env (md, uid) ->
              ...
              | Some id ->
                  ...
                  in
                  Env.add_module_declaration ~check:true id Mp_present mdecl env)
            env decls
        in
        let bindings2 = check_recmodule_inclusion newenv bindings1 in
        let mbs =
          List.filter_map
            (fun (mb, uid) -> Option.map (fun id -> (id, mb, uid)) mb.mb_id)
            bindings2
        in
    ...
    | Pstr_class cl ->
        ...
        ( Tstr_class
            (List.map
               (fun cls ->
                 (cls.Typeclass.cls_info, cls.Typeclass.cls_pub_methods))
               classes),
          ...
          List.fold_right
            (fun lid (nl, tl) ->
              ...
              | path -> (
                  ...
                  | exception Not_found -> (nl, tl)
                  | decl ->
                      if decl.type_arity > 0 then (nl, tl)
                      else
...
let type_implementation sourcefile outputprefix modulename initial_env ast =
  ...
  Misc.try_finally
    (fun () ->
      ...
      let str, sg, names, finalenv = type_structure initial_env ast in
      ...
      if !Clflags.print_types then (
        ...
        in
        if Sys.file_exists sourceintf then (
          let intf_file =
            try Load_path.find_uncap (modulename ^ ".cmi")
            with Not_found ->
              raise
                (Error
          ...
          let coercion =
            Includemod.compunit initial_env ~mark:Mark_positive sourcefile sg
              intf_file dclsig
          in
          Typecore.force_delayed_checks ();
          ...
          Cmt_format.save_cmt (outputprefix ^ ".cmt") modulename annots
          ...
          if not !Clflags.dont_write_files then (
            ...
            let cmi =
              Env.save_signature ~alerts simple_sg modulename
                (outputprefix ^ ".cmi")
            in
            let annots = Cmt_format.Implementation str in
            Cmt_format.save_cmt (outputprefix ^ ".cmt") modulename annots
...
let package_units initial_env objfiles cmifile modulename =
  ...
  if Sys.file_exists mlifile then (
    if not (Sys.file_exists cmifile) then
      raise
        (Error
           (Location.in_file mlifile, Env.empty, Interface_not_compiled mlifile));
    let dclsig = Env.read_signature modulename cmifile in
    Cmt_format.save_cmt (prefix ^ ".cmt") modulename
      (Cmt_format.Packed (sg, objfiles))
      None initial_env None;
    Includemod.compunit initial_env ~mark:Mark_both "(obtained by packing)" sg
      mlifile dclsig)
  else
    (* Determine imports *)
    ...
    (if not !Clflags.dont_write_files then
     let cmi =
       Env.save_signature_with_imports ~alerts:Misc.Stdlib.String.Map.empty sg
         modulename (prefix ^ ".cmi") imports
     in
     Cmt_format.save_cmt (prefix ^ ".cmt") modulename
       (Cmt_format.Packed (cmi.Cmi_format.cmi_sign, objfiles))
       None initial_env (Some cmi));
    Tcoerce_none
...
let report_error ppf = function
  ...
  | Non_generalizable_class (id, desc) ->
      fprintf ppf
        "@[The type of this class,@ %a,@ contains type variables that cannot \
         be generalized@]"
  ...
  | Incomplete_packed_module ty ->
      fprintf ppf "The type of this packed module contains variables:@ %a"
        type_expr ty
  | Scoping_pack (lid, ty) ->
      fprintf ppf "The type %a in this module cannot be exported.@ " longident
        lid;
  ...
  | Cannot_scrape_package_type p ->
      fprintf ppf
        "The type of this packed module refers to %a, which is missing" path p
  ...
  | Cannot_hide_id
      ...
      in
      fprintf ppf
        "@[<v>Illegal shadowing of included %s %a by %a@ %a:@;\
         <1 2>%s %a came from this include@ %a:@;\
         <1 2>The %s %s has no valid type if %a is shadowed@]"
        ...
        Ident.print shadowed_item_id Location.print_loc user_loc
        (Sig_component_kind.to_string user_kind)
        (Ident.name user_id) Ident.print shadowed_item_id
  | Cannot_hide_id
      (Appears_in_signature
      ...
      fprintf ppf
        "@[<v>The %s %a introduced by this open appears in the signature@ %a:@;\
         <1 2>The %s %s has no valid type if %a is hidden@]" opened_item_kind
        Ident.print opened_item_id Location.print_loc user_loc
        (Sig_component_kind.to_string user_kind)
        (Ident.name user_id) Ident.print opened_item_id
  | Invalid_type_subst_rhs ->
