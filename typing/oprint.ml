...
let all_ident_chars s =
  let rec loop s len i =
    if i < len then if is_ident_char s.[i] then loop s len (i + 1) else false
    else true
  in
...
let pr_vars = print_list pr_var (fun ppf -> fprintf ppf "@ ")

let rec print_out_type ppf = function
  | Otyp_alias (ty, s) -> fprintf ppf "@[%a@ as %a@]" print_out_type ty pr_var s
  | Otyp_poly (sl, ty) ->
...
let collect_functor_arguments mty =
  let rec collect_args acc = function
    | Omty_functor (param, mty_res) -> collect_args (param :: acc) mty_res
  ...
  | Omty_functor _ as t ->
      let rec print_functor ppf = function
        | Omty_functor (Some (None, mty_arg), mty_res) ->
            fprintf ppf "%a ->@ %a" print_simple_out_module_type mty_arg
        ...
        | Omty_functor _ as non_anonymous_functor ->
            ...
            fprintf ppf "@[<2>functor@ %a@]@ ->@ %a"
              (pp_print_list ~pp_sep:pp_print_space print_arg)
              args print_functor rest
        | non_functor -> print_simple_out_module_type ppf non_functor
      in
