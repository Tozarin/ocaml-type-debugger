      ...
      let ty =
        ...
        with Not_found -> (
          ...
          with Not_found ->
            ...
            in
            used_variables :=
              TyVarMap.add name (v, styp.ptyp_loc) !used_variables;
            v)
      in
      ...
      let add_field field =
        ...
        let rf_desc =
          ...
          | Rtag (l, c, stl) ->
              name := None;
              let tl =
                Builtin_attributes.warning_scope rf_attributes (fun () ->
              ...
              let f =
                ...
                | _ -> (
                    if List.length stl > 1 || (c && stl <> []) then
                      raise
                        (Error
                           (styp.ptyp_loc, env, Present_has_conjunction l.txt));
                    match tl with
                    | [] -> Rpresent None
          ...
          | Rinherit sty ->
              ...
              let fl =
                ...
                | { desc = Tvar _ }, Some (p, _) ->
                    raise
                      (Error (sty.ptyp_loc, env, Undefined_type_constructor p))
...
and transl_fields env policy o fields =
  ...
  let add_field { pof_desc; pof_loc; pof_attributes } =
    ...
    let of_desc =
      ...
      | Otag (s, ty1) ->
          let ty1 =
            Builtin_attributes.warning_scope of_attributes (fun () ->
    ...
    (fun name (ty, loc) ->
      ...
      if
        ...
        with _ ->
          ...
          false
      then (
        try r := (loc, v, TyVarMap.find name !type_variables) :: !r
        with Not_found ->
...
let transl_simple_type_univars env styp =
  ...
  {
    typ with
    ctyp_type = instance (Btype.newgenty (Tpoly (typ.ctyp_type, univs)));
  }
...
let report_error env ppf = function
  | Unbound_type_variable name ->
      ...
      fprintf ppf
        "The type variable %s is unbound in this type declaration.@ %a" name
  ...
  | Present_has_no_type l ->
      fprintf ppf
        "@[<v>@[The constructor %s is missing from the upper bound@ (between \
         '<'@ and '>')@ of this polymorphic variant@ but is present in@ its \
         lower bound (after '>').@]@,\
         @[Hint: Either add `%s in the upper bound,@ or remove it@ from the \
         lower bound.@]@]"
        l l
  ...
  | Cannot_quantify (name, v) ->
      ...
      if Btype.is_Tvar v then fprintf ppf "it escapes its scope"
      else if Btype.is_Tunivar v then
        fprintf ppf "it is already bound to another variable"
      else fprintf ppf "it is bound to@ %a" Printtyp.type_expr v;
      fprintf ppf ".@]"
