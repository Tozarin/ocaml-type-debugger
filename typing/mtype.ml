...
let rec strengthen ~aliasable env mty p =
  ...
  | Mty_signature sg -> Mty_signature (strengthen_sig ~aliasable env sg p)
...
and strengthen_sig ~aliasable env sg p =
      ...
      let newdecl =
        ...
        | _ ->
            let manif =
              Some
                (Btype.newgenty
                   (Tconstr (Pdot (p, Ident.name id), decl.type_params, ref Mnil)))
            in
  ...
  | Sig_module (id, pres, md, rs, vis) :: rem ->
      let str = strengthen_decl ~aliasable env md (Pdot (p, Ident.name id)) in
...
let rec make_aliases_absent pres mty =
  ...
  | Mty_signature sg -> (pres, Mty_signature (make_aliases_absent_sig sg))
  | Mty_functor (arg, res) ->
...
and make_aliases_absent_sig sg =
  ...
  | sigelt :: rem -> sigelt :: make_aliases_absent_sig rem
...
and remove_aliases_sig env args sg =
  ...
  | Sig_module (id, pres, md, rs, priv) :: rem ->
      let pres, mty =
        match md.md_type with
        | Mty_alias p when args.exclude id p -> (pres, md.md_type)
