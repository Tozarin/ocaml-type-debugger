...
let is_unboxed = function
  | Same_as_ocaml_repr | Untagged_int -> false
...
let parse_declaration valdecl ~native_repr_args ~native_repr_res =
  ...
  then
    raise (Error (valdecl.pval_loc, Old_style_float_with_native_repr_attribute));
  ...
  if
    ...
    && not
         (List.for_all is_ocaml_repr native_repr_args
         && is_ocaml_repr native_repr_res)
  then raise (Error (valdecl.pval_loc, No_native_primitive_with_repr_attribute));
...
let rec add_native_repr_attributes ty attrs =
  ...
  | Otyp_arrow (label, a, b), attr_opt :: rest ->
      ...
      let a =
        match attr_opt with None -> a | Some attr -> Otyp_attribute (a, attr)
      in
...
let print p osig_val_decl =
  ...
  let attrs =
    if all_unboxed then oattr_unboxed :: attrs
    else if all_untagged then oattr_untagged :: attrs
    else attrs
  in
