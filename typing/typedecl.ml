            ...
            List.iter
              (fun cd ->
                if cd.pcd_res <> None then
                  raise (Error (cd.pcd_loc, Nonrec_gadt)))
              scds
        | _ -> ());
...
let transl_declaration env sdecl (id, uid) =
  ...
  (if raw_status.unboxed && not raw_status.default then
  ...
  (if is_fixed_type sdecl then
   let p, _ =
     try Env.find_type_by_name (Longident.Lident (Ident.name id ^ "#row")) env
  ...
  let rec check ty0 parents ty =
    ...
    if TypeSet.mem ty parents then
      if
        ...
        match ty0.desc with Tconstr (p, _, _) -> Path.same p path | _ -> false
    ...
    if fini then ()
      ...
      | Tconstr (p, _, _) when arg_exn <> None || to_check p -> (
          ...
          with Ctype.Cannot_expand -> Option.iter raise arg_exn)
      ...
      if not (List.memq ty !visited) then (
        ...
        | _ ->
            Btype.iter_type_expr
              (check_regular cpath args prev_exp prev_expansions)
              ty)
    in
...
let transl_type_decl env rec_flag sdecl_list =
  ...
  List.iter
    (fun (id, decl) ->
      check_well_founded_manifest new_env
        (List.assoc id id_loc_list)
        (Path.Pident id) decl)
    decls;
  ...
  List.iter
    (fun (id, decl) ->
      check_well_founded_decl new_env
        (List.assoc id id_loc_list)
        (Path.Pident id) decl to_check)
    decls;
  List.iter
    (check_abbrev_recursion ~orig_env:env new_env id_loc_list to_check)
    tdecls;
  ...
  let args, ret_type, kind =
    ...
    | Pext_rebind lid ->
        ...
        if not (Ctype.equal env true cstr_types ext_types) then
          raise
            (Error
               (lid.loc, Rebind_mismatch (lid.txt, cstr_type_path, type_path)));
...
let transl_type_extension extend env loc styext =
  ...
  let constructors =
    List.map
      (transl_extension_constructor ~scope env type_path type_decl.type_params
  ...
  let check_type acc ty : Path.Set.t =
    ...
    try
      ...
      | Tconstr (p, _, _) ->
          ...
          if tydecl.type_unboxed.default then Path.Set.add p acc else acc
      | _ -> acc
    with Not_found -> acc
  in
...
let transl_value_decl env loc valdecl =
  ...
  let v =
    ...
    | _ ->
        ...
        if
          !Clflags.native_code && prim.prim_arity > 5
...
let transl_with_constraint id row_path ~sig_env ~sig_decl ~outer_env sdecl =
  ...
  let priv =
    ...
    else if arity_ok && sig_decl.type_kind <> Type_abstract then
      sig_decl.type_private
    else sdecl.ptype_private
  in
  if
    arity_ok
    && sig_decl.type_kind <> Type_abstract
    && sdecl.ptype_private = Private
  then Location.deprecated loc "spurious use of private";
  let type_kind, type_unboxed =
    if arity_ok && man <> None then (sig_decl.type_kind, sig_decl.type_unboxed)
  ...
  let new_type_variance =
    ...
    try Typedecl_variance.compute_decl env ~check:true new_sig_decl required
    with Typedecl_variance.Error (loc, err) ->
      raise (Error (loc, Variance err))
  in
...
let explain_unbound_gen ppf tv tl typ kwd pr =
  try
    ...
    fprintf ppf ".@.@[<hov2>In %s@ %a@;<1 -2>the variable %a is unbound@]" kwd
      pr ti Printtyp.marked_type_expr tv
  with Not_found -> ()
...
let report_error ppf = function
  ...
  | Non_regular { definition; used_as; defined_as; expansions } -> (
      ...
      | [] ->
          fprintf ppf
            "@[<hv>This recursive type is not regular.@ The type constructor \
             %s is defined as@;\
             <1 2>type %a@ but it is used as@;\
             <1 2>%a.@ All uses need to match the definition for the recursive \
             type to be regular.@]"
            (Path.name definition) !Oprint.out_type
            (Printtyp.tree_of_typexp false defined_as)
            !Oprint.out_type
            (Printtyp.tree_of_typexp false used_as)
      | _ :: _ ->
          fprintf ppf
            "@[<hv>This recursive type is not regular.@ The type constructor \
             %s is defined as@;\
             <1 2>type %a@ but it is used as@;\
             <1 2>%a@ after the following expansion(s):@;\
             <1 2>%a@ All uses need to match the definition for the recursive \
             type to be regular.@]"
            (Path.name definition) !Oprint.out_type
            (Printtyp.tree_of_typexp false defined_as)
  ...
  | Immediacy (Typedecl_immediacy.Bad_immediacy_attribute violation) ->
      fprintf ppf "@[%a@]" Format.pp_print_text
        ...
        | Type_immediacy.Violation.Not_always_immediate ->
            "Types marked with the immediate attribute must be non-pointer \
             types like int or bool."
        | Type_immediacy.Violation.Not_always_immediate_on_64bits ->
            "Types marked with the immediate64 attribute must be produced \
             using the Stdlib.Sys.Immediate64.Make functor.")
  ...
  | Separability (Typedecl_separability.Non_separable_evar evar) ->
      let pp_evar ppf = function
        ...
        | Some str ->
            fprintf ppf "the existential variable %a" Pprintast.tyvar str
      in
      fprintf ppf
        "@[This type cannot be unboxed because@ it might contain both float \
         and non-float values,@ depending on the instantiation of %a.@ You \
         should annotate it with [%@%@ocaml.boxed].@]"
        pp_evar evar
  | Boxed_and_unboxed ->
