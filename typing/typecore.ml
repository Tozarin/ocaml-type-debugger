...
module Datatype_kind = struct
  ...
  let type_name = function Record -> "record" | Variant -> "variant"
  let label_name = function Record -> "field" | Variant -> "constructor"
end
...
type error =
  ...
  | Pattern_type_clash :
      Ctype.Unification_trace.t * _ pattern_desc option
      -> error
...
let get_gadt_equations_level () =
  match !gadt_equations_level with Some y -> y | None -> assert false
...
let unify_pat_types_return_equated_pairs ?(refine = None) loc env ty ty' =
  try
    ...
    | None ->
        ...
        nothing_equated
  with
  | Unify trace -> raise (Error (loc, !env, Pattern_type_clash (trace, None)))
  | Tags (l1, l2) ->
      raise (Typetexp.Error (loc, !env, Typetexp.Variant_tags (l1, l2)))
  ...
  exists_general_pattern
    {
      f =
        (fun (type k) (p : k general_pattern) ->
          match p.pat_desc with Tpat_variant _ -> true | _ -> false);
    }
    p
...
let split_cases env cases =
  ...
  in
  List.fold_right
    (fun ({ c_lhs; c_guard } as case) (vals, exns) ->
...
let get_constr_type_path ty =
  match (repr ty).desc with Tconstr (p, _, _) -> p | _ -> assert false
...
struct
  ...
  let warn_if_ambiguous warn lid env lbl rest =
    ...
    let expansion =
      Format.asprintf "%t" Printtyp.Conflicts.print_explanations
    in
    if paths <> [] then
      warn lid.loc
  ...
  *)
  let warn_if_disambiguated_name warn lid lbl scope =
    ...
    | _ -> warn lid.loc (Warnings.Disambiguated_name (get_name lbl))
  ...
  let disambiguate ?(warn = Location.prerr_warning)
      ?(filter : nonempty_candidate_filter = Result.ok) usage lid env
      expected_type candidates_in_scope =
    let lbl =
...
let wrap_disambiguate msg ty f x =
  try f x
  with Wrong_name_disambiguation (env, wrong_name) ->
...
let disambiguate_lid_a_list loc closed env expected_type lid_a_list =
  ...
  let process_label lid =
    ...
    let filter : Label.nonempty_candidate_filter =
      disambiguate_label_by_ids closed ids
    in
    Label.disambiguate ~warn ~filter () lid env expected_type scope
  in
  let lbl_a_list =
...
let type_label_a_list ?labels loc closed env type_lbl_a expected_type lid_a_list
    k =
  let lbl_a_list =
...
let rec has_literal_pattern p =
  ...
  | Ppat_type _ | Ppat_var _ | Ppat_unpack _ | Ppat_extension _ ->
      false
  | Ppat_exception p
  ...
  | Ppat_tuple ps | Ppat_array ps -> List.exists has_literal_pattern ps
  | Ppat_record (ps, _) -> List.exists (fun (_, p) -> has_literal_pattern p) ps
  | Ppat_or (p, q) -> has_literal_pattern p || has_literal_pattern q
...
type pattern_checking_mode =
  | Normal  (** We are checking user code. *)
...
let rec find_valid_alternative f pat =
  ...
  | Ppat_or (p1, p2) -> (
      ...
      with Empty_branch | Error _ -> find_valid_alternative f p2)
  | _ -> f pat

let no_explosion = function
  | Normal -> Normal
  | Counter_example info -> Counter_example { info with explosion_fuel = 0 }

let get_splitting_mode = function
  | Normal -> None
  | Counter_example { splitting_mode } -> Some splitting_mode

let enter_nonsplit_or mode =
  match mode with
  | Normal -> Normal
  | Counter_example info ->
      ...
      in
      Counter_example { info with splitting_mode }
...
let pure :
 ...
 fun category pat ->
  match category with Value -> pat | Computation -> as_computation_pattern pat

let only_impure :
    type k.
    k pattern_category -> computation general_pattern -> k general_pattern =
 fun category pat ->
  match category with
  | Value ->
...
let as_comp_pattern :
    type k.
    k pattern_category -> k general_pattern -> computation general_pattern =
 fun category pat ->
  match category with Value -> as_computation_pattern pat | Computation -> pat
...
let rec type_pat :
 ...
 fun category ~no_existentials ~mode ~env sp expected_ty k ->
  Builtin_attributes.warning_scope sp.ppat_attributes (fun () ->
      type_pat_aux category ~no_existentials ~mode ~env sp expected_ty k)
...
and type_pat_aux :
 ...
 fun category ~no_existentials ~mode ~env sp expected_ty k ->
  ...
  let refine =
    match mode with Normal -> None | Counter_example _ -> Some true
  in
  ...
  let rp x =
    let crp (x : k general_pattern) : k general_pattern =
      match category with Value -> rp x | Computation -> rcp x
    in
    if mode = Normal then crp x else x
  in
  let rp k x = k (rp x)
  ...
  | Ppat_any -> (
      ...
      | Counter_example ({ explosion_fuel; _ } as info) -> (
          ...
          match ppat_of_type !env expected_ty with
  ...
  | Ppat_var name ->
      ...
      let id =
        (* PR#7330 *)
        if name.txt = "*extension*" then Ident.create_local name.txt
        else enter_variable loc name ty sp.ppat_attributes
      in
      rvp k
        {
  ...
  | Ppat_alias (sq, name) ->
      ...
      type_pat Value sq expected_ty (fun q ->
          ...
          generalize ty_var;
          let id =
            enter_variable ~is_as_variable:true loc name ty_var
              sp.ppat_attributes
          in
          rvp k
            {
  ...
  | Ppat_construct (lid, sarg) ->
      ...
      let ty_args, ty_res =
        instance_constructor
          ~in_pattern:(env, get_gadt_equations_level ())
          constr
      in
               ...
               let msg =
                 Format.asprintf
                   "typing this pattern requires considering@ %a@ and@ %a@ as \
                    equal.@,\
                    But the knowledge of these types" Printtyp.type_expr t1
  ...
  | Ppat_variant (l, sarg) -> (
      let arg_type =
        match sarg with None -> [] | Some _ -> [ newgenvar () ]
      in
      let row =
        {
      ...
      if l = Parmatch.some_private_tag then
      ...
      let k arg =
        rvp k
          {
            pat_desc =
              Tpat_variant (l, arg, ref { row with row_more = newvar () });
            pat_loc = loc;
            pat_extra = [];
            pat_type = instance expected_ty;
            pat_attributes = sp.ppat_attributes;
  ...
  | Ppat_record (lid_sp_list, closed) -> (
      ...
      let type_label_pat (label_lid, label, sarg) k =
        ...
        (try unify_pat_types ~refine loc env ty_res (instance record_ty)
        ...
        type_pat Value sarg ty_arg (fun arg -> k (label_lid, label, arg))
      in
  ...
  | Ppat_array spl ->
      ...
      let expected_ty = generic_instance expected_ty in
      unify_pat_types ~refine loc env (Predef.type_array ty_elt) expected_ty;
      map_fold_cont
        (fun p -> type_pat Value p ty_elt)
        spl
        (fun pl ->
          rvp k
            {
  ...
  | Ppat_or (sp1, sp2) -> (
      let may_split, must_split =
        ...
        | Some (Refine_or _) -> (true, false)
      in
      let state = save_state env in
      let split_or sp =
        assert may_split;
        let typ pat = type_pat category pat expected_ty k in
        find_valid_alternative
          (fun pat ->
            set_state state env;
            typ pat)
          sp
      in
      if must_split then split_or sp
      else
        ...
        | Error Adds_constraints, Error _ | Error _, Error Adds_constraints ->
            let inside_nonsplit_or =
              ...
              | Some (Refine_or { inside_nonsplit_or }) -> inside_nonsplit_or
            in
            if inside_nonsplit_or then raise Need_backtrack else split_or sp
  ...
  | Ppat_constraint (sp, sty) ->
      ...
      type_pat category sp expected_ty' (fun p ->
          ...
          let p : k general_pattern =
            ...
            | _, p -> { p with pat_type = ty; pat_extra = extra :: p.pat_extra }
          in
          k p)
  | Ppat_type lid ->
...
let type_pat category ?no_existentials ?(mode = Normal)
  ...
  Misc.protect_refs
    ...
    (fun () ->
      type_pat category ~no_existentials ~mode ~env sp expected_ty (fun x -> x))

(* this function is passed to Partial.parmatch
   to type check gadt nonexhaustiveness *)
let partial_pred ~lev ~splitting_mode ?(explode = 0) env expected_ty constrs
    labels p =
  let env = ref env in
  let state = save_state env in
  let mode =
    Counter_example
      { splitting_mode; explosion_fuel = explode; constrs; labels }
  in
  try
    reset_pattern true;
...
let check_unused ?(lev = get_current_level ()) env expected_ty cases =
  Parmatch.check_unused
    (fun refute constrs labels spat ->
      match
        partial_pred ~lev ~splitting_mode:Backtrack_or ~explode:5 env
          expected_ty constrs labels spat
      with
...
let type_pattern_list category no_existentials env spatl expected_tys allow =
  reset_pattern allow;
  let new_env = ref env in
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  ...
  let nv = newvar () in
  let pat =
    type_pat Value ~no_existentials:In_class_args (ref val_env) spat nv
  in
...
let type_self_pattern cl_num privty val_env met_env par_env spat =
  ...
  let nv = newvar () in
  let pat =
    type_pat Value ~no_existentials:In_self_pattern (ref val_env) spat nv
  in
  List.iter (fun f -> f ()) (get_ref pattern_force);
...
let rec is_nonexpansive exp =
  match exp.exp_desc with
  | Texp_ident _ | Texp_constant _ | Texp_unreachable | Texp_function _
  | Texp_array [] ->
      true
  ...
  | Texp_apply
      ...
      is_nonexpansive e
  | Texp_array (_ :: _)
  | Texp_apply _ | Texp_try _ | Texp_setfield _ | Texp_while _ | Texp_for _
  | Texp_send _ | Texp_instvar _ | Texp_setinstvar _ | Texp_override _
  | Texp_letexception _ | Texp_letop _ | Texp_extension_constructor _ ->
      false
...
and is_nonexpansive_opt = function None -> true | Some e -> is_nonexpansive e
  ...
  let rec f delay =
              ...
              | _ ->
                  let loc =
                    match
                      List.find_opt
                        (function
                          | Texp_constraint _, _, _ -> true | _ -> false)
                        exp_extra
                    with
                    | Some (_, loc, _) -> loc
                    | None -> exp_loc
    ...
    | Tarrow _, _ ->
        let rec check { exp_desc; exp_loc; exp_extra; _ } =
          if
            List.exists
              (function Texp_constraint _, _, _ -> true | _ -> false)
              exp_extra
          then check_statement ()
          else
            match exp_desc with
            | Texp_ident _ | Texp_constant _ | Texp_tuple _ | Texp_construct _
            | Texp_variant _ | Texp_record _ | Texp_field _ | Texp_setfield _
            | Texp_array _ | Texp_while _ | Texp_for _ | Texp_instvar _
            | Texp_setinstvar _ | Texp_override _ | Texp_assert _ | Texp_lazy _
        ...
        in
        check exp
    | Tvar _, _ -> if delay then add_delayed_check (fun () -> f false)
    | _ -> check_statement ()
  in
  f true
...
let exists_ppat f p =
  ...
  in
  match loop p with exception Found -> true | () -> false

let contains_polymorphic_variant p =
  exists_ppat
    (function
      | { ppat_desc = Ppat_variant _ | Ppat_type _ } -> true | _ -> false)
    p
...
let may_contain_gadts p =
  exists_ppat
    (function { ppat_desc = Ppat_construct (_, _) } -> true | _ -> false)
    p
          ...
          | Tpat_variant (s, arg, row) ->
              ...
              then ()
                ...
                let row' =
                  {
                    row_fields =
                      [ (s, Reither (arg = None, ty_arg, true, ref None)) ];
                    row_more = newvar ();
                    row_bound = ();
                    row_closed = false;
                    row_fixed = None;
...
let unify_exp env exp expected_ty =
  ...
  try unify_exp_types loc env exp.exp_type expected_ty
...
and with_explanation explanation f =
  ...
  | Some explanation -> (
      try f ()
      with
      | Error (loc', env', Expr_type_clash (trace', None, exp'))
...
and type_expect_ ?in_function ?(recarg = Rejected) env sexp
  ...
  let rue exp =
    with_explanation (fun () -> unify_exp env (re exp) (instance ty_expected));
    exp
  in
  match sexp.pexp_desc with
  | Pexp_ident lid ->
  ...
  | Pexp_constant (Pconst_string (str, _, _) as cst) ->
      ...
      let fmt6_path =
        Path.(
          Pdot
            ( Pident (Ident.create_persistent "CamlinternalFormatBasics"),
              "format6" ))
      in
      let is_format =
  ...
  | Pexp_let (rec_flag, spat_sexp_list, sbody) ->
      ...
      let () =
        if rec_flag = Recursive then check_recursive_bindings env pat_exp_list
      in
      re
        {
  ...
  | Pexp_function caselist ->
      type_function ?in_function loc sexp.pexp_attributes env
        ty_expected_explained Nolabel caselist
  ...
  | Pexp_match (sarg, caselist) ->
      ...
      let cases, partial =
        type_cases Computation env arg.exp_type ty_expected_explained true loc
          caselist
      in
      re
        {
  ...
  | Pexp_try (sbody, caselist) ->
      ...
      let cases, _ =
        type_cases Value env Predef.type_exn ty_expected_explained false loc
          caselist
      in
      re
        {
  ...
  | Pexp_tuple sexpl ->
      ...
      let expl =
        List.map2
          (fun body ty -> type_expect env body (mk_expected ty))
          sexpl subtypes
      in
      re
        {
                ...
                (fun lbl ->
                  ...
                  | exception Not_found ->
                      ...
                      let rec missing_labels n = function
                        | [] -> []
                        | lbl :: rem ->
                            if List.mem n present_indices then
  ...
  | Pexp_setfield (srecord, lid, snewval) ->
      let record, label, expected_type = type_label_access env srecord lid in
      let ty_record =
        if expected_type = None then newvar () else record.exp_type
      in
      let label_loc, label, newval =
        type_label_exp false env loc ty_record (lid, label, snewval)
      in
  ...
  | Pexp_array sargl ->
      ...
      with_explanation (fun () ->
          unify_exp_types loc env to_unify (generic_instance ty_expected));
      let argl =
        List.map (fun sarg -> type_expect env sarg (mk_expected ty)) sargl
      in
      re
        {
  ...
  | Pexp_coerce (sarg, sty, sty') ->
      ...
      let arg, ty', cty, cty' =
        ...
        | None ->
            ...
            | ( Texp_ident (_, _, { val_kind = Val_self _ }),
                (path, r) :: _,
                Tconstr (path', _, _) )
      ...
      rue
        {
          ...
          exp_env = env;
          exp_extra =
            (Texp_coerce (cty, cty'), loc, sexp.pexp_attributes)
            :: arg.exp_extra;
        }
  | Pexp_send (e, { txt = met }) -> (
      if !Clflags.principal then begin_def ();
        ...
        let meth, exp, typ =
          ...
          | Texp_ident (_path, _, { val_kind = Val_self (meths, _, _, privty) })
            ->
              ...
              let id, typ = filter_self_method env met Private meths privty in
            ...
            -> (
              ...
              match
                ( Env.find_value_by_name
                    (Longident.Lident ("selfpat-" ^ cl_num))
                    env,
                  Env.find_value_by_name
              ...
              | ( (_, ({ val_kind = Val_self (meths, _, _, privty) } as desc)),
                  ...
                  let exp =
                    Texp_apply
                      ( {
                          exp_desc =
                            Texp_ident (Path.Pident method_id, lid, method_desc);
                          exp_loc = loc;
                          exp_extra = [];
              ...
              | _ -> assert false)
          | _ ->
              (Tmeth_name met, None, filter_method env met Public obj.exp_type)
        in
        if !Clflags.principal then (
          end_def ();
  ...
  | Pexp_setinstvar (lab, snewval) -> (
      ...
      match mut with
      | Mutable ->
          let newval = type_expect env snewval (mk_expected (instance ty)) in
          let path_self, _ =
            Env.find_value_by_name (Longident.Lident ("self-" ^ cl_num)) env
          in
          rue
        ...
        List.fold_right
          (fun (lab, _) l ->
            if List.exists (fun l -> l.txt = lab.txt) l then
              raise (Error (loc, env, Value_multiply_overridden lab.txt));
            lab :: l)
          lst []
      in
      match
        ...
        with Not_found -> raise (Error (loc, env, Outside_class))
      with
      | ( (_, { val_type = self_ty; val_kind = Val_self (_, vars, _, _) }),
          ...
          let type_override (lab, snewval) =
            ...
            with Not_found ->
              ...
              raise
                (Error (loc, env, Unbound_instance_variable (lab.txt, vars)))
          in
  ...
  | Pexp_assert e ->
      ...
      let exp_type =
        ...
        | Texp_construct (_, { cstr_name = "false" }, _) -> instance ty_expected
  ...
  | Pexp_poly (sbody, sty) ->
      ...
      let exp =
        ...
        | Tvar _ ->
            ...
            let exp =
              { exp with exp_type = newty (Tpoly (exp.exp_type, [])) }
            in
  ...
  | Pexp_newtype ({ txt = name }, sbody) ->
      let ty =
        if Typetexp.valid_tyvar_name name then newvar ~name () else newvar ()
      in
      (* remember original level *)
      begin_def ();
  ...
  | Pexp_letop { let_ = slet; ands = sands; body = sbody } ->
      ...
      let ty_op =
        newty
          (Tarrow
             ( Nolabel,
               ty_andops,
               newty (Tarrow (Nolabel, ty_func, ty_result, Cok)),
               Cok ))
      in
      (try unify env op_type ty_op
       with Unify trace ->
         raise
           (Error (let_loc, env, Letop_type_clash (slet.pbop_op.txt, trace))));
      if !Clflags.principal then (
        end_def ();
      ...
      let cases, partial =
        type_cases Value env ty_params
          (mk_expected ty_func_result)
          true loc [ scase ]
      in
      let body = match cases with [ case ] -> case | _ -> assert false in
      ...
      in
      let desc = Texp_letop { let_; ands; param; body; partial } in
      rue
        {
          exp_desc = desc;
...
and type_function ?in_function loc attrs env ty_expected_explained l caselist =
  let { ty = ty_expected; explanation } = ty_expected_explained in
  let loc_fun, ty_fun =
    match in_function with Some p -> p | None -> (loc, instance ty_expected)
  in
  let separate = !Clflags.principal || Env.has_local_constraints env in
  ...
  if separate then (
    ...
    generalize_structure ty_res);
  let cases, partial =
    type_cases Value ~in_function:(loc_fun, ty_fun) env ty_arg
...
and type_label_access env srecord lid =
  ...
  let label =
    wrap_disambiguate "This expression has" (mk_expected ty_exp)
      (Label.disambiguate () lid env expected_type)
      labels
  in
...
and type_format loc str env =
    ...
    CamlinternalFormatBasics.(
      CamlinternalFormat.(
        ...
        let mk_constr name args =
          let lid =
            Longident.(Ldot (Lident "CamlinternalFormatBasics", name))
          in
          let arg =
            match args with
            | [] -> None
            | [ e ] -> Some e
            | _ :: _ :: _ -> Some (mk_exp_loc (Pexp_tuple args))
          in
        ...
        and mk_ignored :
         ...
         fun ign ->
          ...
          | Ignored_nativeint (iconv, pad_opt) ->
              mk_constr "Ignored_nativeint"
                [ mk_iconv iconv; mk_int_opt pad_opt ]
          | Ignored_int64 (iconv, pad_opt) ->
              mk_constr "Ignored_int64" [ mk_iconv iconv; mk_int_opt pad_opt ]
          | Ignored_float (pad_opt, prec_opt) ->
              mk_constr "Ignored_float"
                [ mk_int_opt pad_opt; mk_int_opt prec_opt ]
          | Ignored_bool pad_opt ->
              mk_constr "Ignored_bool" [ mk_int_opt pad_opt ]
          | Ignored_format_arg (pad_opt, fmtty) ->
              mk_constr "Ignored_format_arg"
        ...
        and mk_padding : type x y. (x, y) padding -> Parsetree.expression =
         fun pad ->
          ...
          | Lit_padding (s, w) ->
              mk_constr "Lit_padding" [ mk_side s; mk_int w ]
        ...
        and mk_fmt :
            type a b c d e f. (a, b, c, d, e, f) fmt -> Parsetree.expression =
         fun fmt ->
          match fmt with
          ...
          | Formatting_lit (fmting, rest) ->
              mk_constr "Formatting_lit"
                [ mk_formatting_lit fmting; mk_fmt rest ]
          | Formatting_gen (fmting, rest) ->
              mk_constr "Formatting_gen"
                [ mk_formatting_gen fmting; mk_fmt rest ]
  ...
  with Failure msg -> raise (Error (loc, env, Invalid_format msg))
...
and type_label_exp create env loc ty_expected (lid, label, sarg) =
  ...
  if separate then (
    ...
    generalize_structure ty_arg;
    generalize_structure ty_res);
  (try unify env (instance ty_res) (instance ty_expected)
   with Unify trace ->
...
and type_argument ?explanation ?recarg env sarg ty_expected' ty_expected =
        ...
        if args = [] then texp
          ...
          let var_pair name ty =
            ...
            ( {
              ...
              },
              {
                ...
                exp_desc =
                  Texp_ident
                    (Path.Pident id, mknoloc (Longident.Lident name), desc);
              } )
          in
          ...
          let func texp =
            ...
            let param = name_cases "param" cases in
            {
              ...
              exp_desc =
                Texp_function
                  { arg_label = Nolabel; param; cases; partial = Total };
            }
          in
  ...
  | _ ->
      let texp =
        type_expect ?recarg env sarg (mk_expected ?explanation ty_expected')
      in
      unify_exp env texp ty_expected;
      texp
...
and type_application env funct sargs =
  ...
  let type_unknown_arg (ty_fun, typed_args) (lbl, sarg) =
    let ty_arg, ty_res =
      ...
      | Tvar _ ->
          ...
          if ty_fun.level >= t1.level && not (is_prim ~name:"%identity" funct)
          then
            Location.prerr_warning sarg.pexp_loc Warnings.Ignored_extra_argument;
      ...
      | td -> (
          ...
          | Tarrow _ ->
              if !Clflags.classic || not (has_label lbl ty_fun) then
                raise
                  (Error
                     (sarg.pexp_loc, env, Apply_wrong_label (lbl, ty_res, false)))
    ...
    let arg () =
      ...
      if is_optional lbl then unify_exp env arg (type_option (newvar ()));
      arg
    in
  ...
  let rec type_args args ty_fun ty_fun0 sargs =
    ...
    | ( ({ desc = Tarrow (l, ty, ty_fun, com); level = lv } as ty_fun'),
      ...
      when sargs <> [] && commu_repr com = Cok ->
        let may_warn loc w =
          if (not !warned) && !Clflags.principal && lv <> generic_level then (
            warned := true;
    ...
    | _ ->
        ...
        let args =
          (* Force typing of arguments.
             ...
             incorrect. *)
          List.map
            (function l, None -> (l, None) | l, Some f -> (l, Some (f ())))
            (List.rev typed_args)
        in
  ...
  let is_ignore funct =
    ...
    &&
    try
      ignore (filter_arrow env (instance funct.exp_type) Nolabel);
      true
    with Unify _ -> false
  in
  match sargs with
...
and type_statement ?explanation env sexp =
  ...
  if is_Tvar ty && ty.level > tv.level then
    Location.prerr_warning (final_subexpression exp).exp_loc
      Warnings.Nonreturning_statement;
  if !Clflags.strict_sequence then (
    let expected_ty = instance Predef.type_unit in
    with_explanation explanation (fun () -> unify_exp env exp expected_ty);
    exp)
  else (
      ...
      re
        {
          exp_desc =
            Texp_letmodule
              (Some id, { name with txt = Some name.txt }, pres, modl, body);
          exp_loc;
          exp_attributes;
          exp_extra = [];
          exp_type = ty;
...
and type_cases :
    ...
    Parsetree.case list ->
    k case list * partial =
 fun category ?in_function env ty_arg ty_res_explained partial_flag loc caselist ->
  ...
  let ty_arg =
    if (may_contain_gadts || erase_either) && not !Clflags.principal then
      correct_levels ty_arg
    else ty_arg
  in
  let rec is_var spat =
  ...
  let cases =
    List.map
      (fun {
        ...
        let ext_env =
          if contains_gadt then do_copy_types ext_env else ext_env
        in
        let ext_env =
        ...
        in
        let exp =
          type_unpacks ?in_function ext_env unpacks pc_rhs
      ...
      half_typed_cases
  in
  (if !Clflags.principal || does_contain_gadt then
  ...
  let val_cases, exn_cases =
    ...
    | Computation -> split_cases env cases
  in
  if val_cases = [] && exn_cases <> [] then
    raise (Error (loc, env, No_value_clauses));
  let partial =
    if partial_flag then check_partial ~lev env ty_arg_check loc val_cases
    else Partial
  in
  ...
  in
  if contains_polyvars then add_delayed_check (fun () -> unused_check true)
  else
    (* Check for unused cases, do not delay because of gadts *)
    unused_check false;
  if may_contain_gadts then (
    end_def ();
...
and type_let ?(check = fun s -> Warnings.Unused_var s)
    ?(check_strict = fun s -> Warnings.Unused_var_strict s) existential_context
    ...
    List.map
      (fun { pvb_pat = spat; pvb_expr = sexp; pvb_attributes = attrs } ->
        ( attrs,
          ...
          | (_, Pexp_coerce (_, _, sty) | _, Pexp_constraint (_, sty))
  ...
  let exp_env =
    ...
    else if List.for_all sexp_is_fun spat_sexp_list then
        ...
        | Tpoly (ty, tl) ->
            ...
            let exp =
              Builtin_attributes.warning_scope pvb_attributes (fun () ->
                  ...
                  else type_expect exp_env sexp (mk_expected ty'))
            in
            (exp, Some vars)
        | _ ->
            let exp =
              Builtin_attributes.warning_scope pvb_attributes (fun () ->
                  ...
                  else type_expect exp_env sexp (mk_expected pat.pat_type))
            in
            (exp, None))
      spat_sexp_list pat_slot_list
  in
  current_slot := None;
  (if is_recursive && not !rec_needed then
    ...
    (function
      | { vb_pat = { pat_desc = Tpat_any; pat_extra; _ }; vb_expr; _ } ->
          if
            not
              (List.exists
                 (function Tpat_constraint _, _, _ -> true | _ -> false)
                 pat_extra)
          then check_partial_application false vb_expr
      | _ -> ())
    l;
  (l, new_env, unpacks)
...
and type_andops env sarg sands expected_ty =
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | { pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _ } :: rest ->
        ...
        (try unify env (instance ty_result) (instance expected_ty)
         with Unify trace ->
...
let type_binding env rec_flag spat_sexp_list =
  ...
  let pat_exp_list, new_env, _unpacks =
    type_let
      ...
      At_toplevel env rec_flag spat_sexp_list false
  in
...
let type_clash_of_trace trace =
  Ctype.Unification_trace.(
    explain trace (fun ~prev:_ -> function Diff diff -> Some diff | _ -> None))
...
let report_literal_type_constraint expected_type const =
  ...
  in
  let suffix =
    if Path.same expected_type Predef.path_int32 then Some 'l'
    else if Path.same expected_type Predef.path_int64 then Some 'L'
    else if Path.same expected_type Predef.path_nativeint then Some 'n'
    else if Path.same expected_type Predef.path_float then Some '.'
    else None
  in
...
let report_literal_type_constraint const = function
  | Some
      Unification_trace.{ expected = { t = { desc = Tconstr (typ, [], _) } } }
    ->
...
let report_pattern_type_clash_hints (type k) (pat : k pattern_desc option) diff
    =
  match pat with
...
let report_unification_error ~loc ?sub env trace ?type_expected_explanation txt1
    txt2 =
  Location.error_of_printer ~loc ?sub
    (fun ppf () ->
      Printtyp.report_unification_error ppf env trace ?type_expected_explanation
        txt1 txt2)
    ()
...
let report_error ~loc env = function
  ...
  | Apply_non_function typ -> (
      ...
      | Tarrow _ ->
          Location.errorf ~loc
            "@[<v>@[<2>This function has type@ %a@]@ @[It is applied to too \
             many arguments;@ %s@]@]"
  ...
  | Label_missing labels ->
      let print_labels ppf =
        List.iter (fun lbl -> fprintf ppf "@ %s" (Ident.name lbl))
      in
        ...
        (fun ppf () ->
          Printtyp.wrap_printing_env ~error:true env (fun () ->
              ...
              if Path.is_constructor_typath type_path then
                fprintf ppf
                  "@[The field %s is not part of the record argument for the \
                   %a constructor@]"
                  name.txt Printtyp.type_path type_path
              else
                fprintf ppf
                  "@[@[<2>%s type@ %a%t@]@ The %s %s does not belong to type \
                   %a@]"
      ...
      Location.error_of_printer ~loc
        (fun ppf () ->
          Printtyp.report_ambiguous_type_error ppf env tp tpl
            (function
              | ppf ->
                  fprintf ppf "The %s %a@ belongs to the %s type" name longident
                    lid type_name)
            (function
              | ppf ->
                  fprintf ppf
                    "The %s %a@ belongs to one of the following %s types:" name
                    longident lid type_name)
            (function
              | ppf ->
  ...
  | Undefined_inherited_method (me, valid_methods) ->
      Location.error_of_printer ~loc
        ...
        ()
  | Virtual_class cl ->
      Location.errorf ~loc "Cannot instantiate the virtual class %a" longident
        cl
  ...
  | Abstract_wrong_label (l, ty, explanation) ->
      let label_mark = function
        ...
        | l ->
            sprintf "but its first argument is labelled %s"
  ...
  | Scoping_let_module (id, ty) ->
      Location.errorf ~loc
        "This `let module' expression has type@ %a@ In this type, the locally \
         bound module name %s escapes its scope"
  ...
  | Incoherent_label_order ->
      Location.errorf ~loc
        "This function is applied to arguments@ in an order different from \
  ...
  | Unexpected_existential (reason, name, types) -> (
      let reason_str =
        ...
        | In_class_def ->
            "Existential types are not allowed in bindings inside class \
             definition"
        | In_self_pattern ->
        ...
        | In_rec -> "Existential types are not allowed in recursive bindings"
        | With_attributes ->
            "Existential types are not allowed in presence of attributes"
      in
      match List.find (fun ty -> ty <> "$" ^ name) types with
      | example ->
          Location.errorf ~loc
  ...
  | Letop_type_clash (name, trace) ->
      report_unification_error ~loc env trace
        (function ppf -> fprintf ppf "The operator %s has type" name)
        (function ppf -> fprintf ppf "but it was expected to have type")
  ...
  | Bindings_type_clash trace ->
      report_unification_error ~loc env trace
        (function ppf -> fprintf ppf "These bindings have type")
        (function ppf -> fprintf ppf "but bindings were expected of type")
