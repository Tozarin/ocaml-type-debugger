...
let attrs s x =
  let x =
    if s.for_saving && not !Clflags.keep_docs then List.filter is_not_doc x
    else x
  in
          ...
          | Tvariant row -> (
              ...
              | _ -> (
                  let dup =
                    ...
                    || match more.desc with Tconstr _ -> true | _ -> false
                  in
                  ...
                  (* Return a new copy *)
                  let row =
                    copy_row (typexp copy_scope s) true row (not dup) more'
                  in
...
let constructor_arguments copy_scope s = function
  | Cstr_tuple l -> Cstr_tuple (List.map (typexp copy_scope s) l)
...
let rec class_type copy_scope s = function
  | Cty_constr (p, tyl, cty) ->
      ...
      Cty_constr (p', tyl', cty')
  | Cty_signature sign -> Cty_signature (class_signature copy_scope s sign)
  | Cty_arrow (l, ty, cty) ->
...
let rec modtype scoping s = function
  | Mty_ident p as mty -> (
      match p with
      | Pident id -> ( try Ident.Map.find id s.modtypes with Not_found -> mty)
      | Pdot (p, n) -> Mty_ident (Pdot (module_path s p, n))
      | Papply _ -> fatal_error "Subst.modtype")
  | Mty_signature sg -> Mty_signature (signature scoping s sg)
  | Mty_functor (Unit, res) -> Mty_functor (Unit, modtype scoping s res)
  ...
  | Mty_functor (Named (Some id, arg), res) ->
      let id' = Ident.rename id in
      Mty_functor
        ( Named (Some id', (modtype scoping s) arg),
...
and signature_item scoping s comp =
  For_copy.with_scope (fun copy_scope ->
      signature_item' copy_scope scoping s comp)
