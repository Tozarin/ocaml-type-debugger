...
let rec remove id = function
  ...
  | Node (l, k, r, h) as m ->
      let c = String.compare (name id) (name k.ident) in
      if c = 0 then
        match k.previous with None -> merge l r | Some k -> Node (l, k, r, h)
      else if c < 0 then
        let ll = remove id l in
        if l == ll then m else balance ll k r
      else
        let rr = remove id r in
        if r == rr then m else balance l k rr
...
let rec find_name n = function
  ...
  | Node (l, k, r, _) ->
      ...
      if c = 0 then (k.ident, k.data) else find_name n (if c < 0 then l else r)
