...
let classify_expression : Typedtree.expression -> sd =
  ...
  *)
  let rec classify_expression env e =
    ...
    | Texp_record
        {
          representation = Record_unboxed _;
    ...
    | Texp_apply ({ exp_desc = Texp_ident (_, _, vd) }, _) when is_ref vd ->
        Static
    | Texp_apply (_, args) when List.exists is_abstracted_arg args -> Static
    | Texp_apply _ -> Dynamic
    | Texp_for _ | Texp_constant _ | Texp_new _ | Texp_instvar _ | Texp_tuple _
    | Texp_array _ | Texp_variant _ | Texp_setfield _ | Texp_while _
    | Texp_setinstvar _ | Texp_pack _ | Texp_object _ | Texp_function _
    | Texp_lazy _ | Texp_unreachable | Texp_extension_constructor _ ->
        Static
    | Texp_match _ | Texp_ifthenelse _ | Texp_send _ | Texp_field _
...
let rec expression : Typedtree.expression -> term_judg =
 fun exp ->
  ...
  | Texp_tuple exprs -> list expression exprs << Guard
  | Texp_array exprs ->
      let array_mode =
  ...
  | Texp_construct (_, desc, exprs) ->
      let access_constructor =
        ...
        | Cstr_extension (pth, _) -> path pth << Dereference
        | _ -> empty
      in
      let m' =
        match desc.cstr_tag with
        | Cstr_unboxed -> Return
        | Cstr_constant _ | Cstr_block _ | Cstr_extension _ -> Guard
      in
      join [ access_constructor; list expression exprs << m' ]
  | Texp_variant (_, eo) ->
      (*
        G |- e: m[Guard]
        ------------------   -----------
  ...
  | Texp_record { fields = es; extended_expression = eo; representation = rep }
    ->
      let field_mode =
        ...
        | Record_regular | Record_inlined _ | Record_extension _ -> Guard
      in
      let field (_label, field_def) =
        match field_def with
        | Kept _ -> empty
        | Overridden (_, e) -> expression e
      in
      join [ array field es << field_mode; option expression eo << Dereference ]
  ...
  | Texp_while (cond, body) ->
      ...
      *)
      join [ expression cond << Dereference; expression body << Guard ]
  | Texp_send (e1, _, eo) ->
      (*
        G |- e: m[Dereference]
        ...
        G |- e#x: m
      *)
      join [ expression e1 << Dereference; option expression eo << Dereference ]
  | Texp_field (e, _, _) ->
      (*
        G |- e: m[Dereference]
        -----------------------
  ...
  | Texp_letop { let_; ands; body; _ } ->
      ...
      join
        [
          list binding_op (let_ :: ands) << Dereference; case_env body << Delay;
        ]
  | Texp_unreachable ->
      (*
        ----------
    ...
    | Recursive ->
        ...
        *)
        ...
        let rec transitive_closure env =
          ...
          if List.for_all2 Env.equal env env' then env'
