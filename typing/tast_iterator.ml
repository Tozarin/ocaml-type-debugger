...
let pat_extra sub (e, _loc, _attrs) =
  match e with
  | Tpat_type _ -> ()
  | Tpat_unpack -> ()
...
let expr sub { exp_extra; exp_desc; exp_env; _ } =
  ...
  | Texp_let (rec_flag, list, exp) ->
      ...
      sub.expr sub exp
  | Texp_function { cases; _ } -> List.iter (sub.case sub) cases
  | Texp_apply (exp, list) ->
      sub.expr sub exp;
  ...
  | Texp_record { fields; extended_expression; _ } ->
      Array.iter
        (function
          | _, Kept _ -> () | _, Overridden (_, exp) -> sub.expr sub exp)
        fields;
  ...
  | Texp_override (_, list) -> List.iter (fun (_, _, e) -> sub.expr sub e) list
