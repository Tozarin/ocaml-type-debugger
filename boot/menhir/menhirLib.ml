module General = struct
  ...
  let rec uniq1 cmp x ys =
    match ys with
    | [] -> []
    | y :: ys -> if cmp x y = 0 then uniq1 compare x ys else y :: uniq1 cmp y ys
    ...
    include
      IncrementalEngine.INCREMENTAL_ENGINE
        with type token := token
         and type 'a lr1state =
          state (* useful for us; hidden from the end user *)
...
module Engine = struct
  ...
  module Make (T : TABLE) = struct
    ...
    let rec run env please_discard : semantic_value checkpoint =
      ...
      if please_discard then InputNeeded env
      else check_for_default_reduction env
    ...
    and check_for_error_token env =
      ...
      if env.error then (
        if log then Log.resuming_error_handling ();
        HandlingError env)
      else
        ...
        T.action env.current (* determines a row *)
          (T.token2terminal token) (* determines a column *)
          (T.token2value token) shift (* shift continuation *)
    ...
    and shift env (please_discard : bool) (terminal : terminal)
      ...
      let stack =
        { state = env.current; semv = value; startp; endp; next = env.stack }
      in
    ...
    and announce_reduce env (prod : production) =
      if T.is_start prod then accept env prod else AboutToReduce (env, prod)
    ...
    and error env =
      ...
      T.action env.current (* determines a row *)
        ...
        T.error_value error_shift (* shift continuation *)
    ...
    and error_fail env =
      ...
      if next == cell then (* The stack is empty. Die. *)
        Rejected
      else
    ...
    let offer :
          'a. 'a checkpoint -> token * position * position -> 'a checkpoint =
      function
    ...
    let resume : 'a. 'a checkpoint -> 'a checkpoint = function
      | HandlingError env -> Obj.magic error env
      | Shifting (_, env, please_discard) -> Obj.magic run env please_discard
    ...
    let rec shifts checkpoint =
      ...
      | HandlingError _ ->
          (* The parser fails, which means it rejects the terminal symbol
             that we have fed it. *)
          None
      | InputNeeded _ | Accepted _ | Rejected ->
      ...
      lazy
        ((* The stack is empty iff the top stack cell is its own successor. In
         ...
         if next == cell then Nil
           ...
           let element = Element (current, cell.semv, cell.startp, cell.endp) in
           Cons (element, stack next cell.state))

    let stack env : element stream = stack env.stack env.current
    ...
    let top env : element option =
      let cell = env.stack in
      let next = cell.next in
      if next == cell then None
      else Some (Element (env.current, cell.semv, cell.startp, cell.endp))
    ...
    let equal env1 env2 =
      env1.stack == env2.stack && number env1.current = number env2.current
    ...
    let state_has_default_reduction (state : _ lr1state) : bool =
      T.default_reduction state (fun _env _prod -> true) (fun _env -> false) ()

    let env_has_default_reduction env = state_has_default_reduction env.current
    ...
    let pop (env : 'a env) : 'a env option =
      let cell = env.stack in
      let next = cell.next in
      if next == cell then (* The stack is empty. *)
        None
      else
    ...
    let input_needed (env : 'a env) : 'a checkpoint = InputNeeded env

    (* The following functions are compositions of [top] and [pop]. *)

    let rec pop_many i env =
      if i = 0 then Some env
      else match pop env with None -> None | Some env -> pop_many (i - 1) env
...
module ErrorReports = struct
  ...
  type 'a content = Zero | One of 'a | Two of 'a * (* most recent: *) 'a
...
module PackedIntArray = struct
  ...
  let magnitude (v : int) =
    if v < 0 then Sys.word_size
    else
      let rec check k max =
        (* [max] equals [2^k] *)
        if max <= 0 || v < max then k
  ...
  let pack (a : int array) : t =
    ...
    let k = Array.fold_left (fun k v -> max k (magnitude v)) 1 a in
    ...
    if k <= 8 then (
      ...
      let next () =
        let ii = !i in
        if ii = m then 0 (* ran off the end, pad with zeroes *)
        else
          let v = a.(ii) in
          i := ii + 1;
          v
  ...
  let get ((k, s) : t) (i : int) : int =
    ...
    | _ ->
        assert (k = 32);
        (* 64 bits unlikely, not supported *)
        let j = 4 * i in
        (((((read s j lsl 8) + read s (j + 1)) lsl 8) + read s (j + 2)) lsl 8)
        + read s (j + 3)
...
module RowDisplacement = struct
  ...
  type 'a table = int array * (* displacement *)
                  'a array (* data *)
  ...
  let encode (displacement : int) : int =
    if displacement >= 0 then displacement lsl 1 else (-displacement lsl 1) + 1

  let decode (displacement : int) : int =
    if displacement land 1 = 0 then displacement lsr 1 else -(displacement lsr 1)
  ...
  let compress (equal : 'a -> 'a -> bool) (insignificant : 'a -> bool)
      (dummy : 'a) (m : int) (n : int) (t : 'a array array) : 'a table =
    (* Be defensive. *)
    ...
    let sparse (i : int) (line : 'a array) :
        int * int * 'a row (* index, rank, row *) =
      let rec loop (j : int) (rank : int) (row : 'a row) =
        if j < 0 then (i, rank, row)
        else
          let x = line.(j) in
          if insignificant x then loop (j - 1) rank row
          else loop (j - 1) (1 + rank) ((j, x) :: row)
      in
    ...
    Array.fast_sort
      (fun (_, rank1, _) (_, rank2, _) -> compare rank2 rank1)
      rows;
      ...
      let rec loop = function
        ...
        | (j, x) :: row ->
            ...
            if k + j >= d then true
              ...
              if insignificant y || equal x y then loop row else false
      in
    ...
    (* Note that displacements may be negative. This means that, for
       ...
       elements. *)
    let rec fit k row : int = if fits k row then k else fit (k + 1) row in

    let fit row =
      match row with [] -> 0 (* irrelevant *) | (j, _) :: _ -> fit (-j) row
    in
...
module LinearizedArray = struct
  ...
  let make (a : 'a array array) : 'a t =
    ...
    let entry =
      Array.init (n + 1) (fun i ->
          let s = !size in
          if i < n then size := s + Array.length a.(i);
          s)
    in
    assert (entry.(n) = !size);
  ...
  let rec read_interval_via get_data i j =
    if i = j then [] else get_data i :: read_interval_via get_data (i + 1) j
...
module TableFormat = struct
  ...
  module type TABLES = sig
    ...
    val error :
      int (* width of the bitmap *)
      * string (* second component of [PackedIntArray.t] *)
...
module InspectionTableInterpreter = struct
  ...
  struct
    ...
    let decode_symbol (symbol : int) : IT.xsymbol =
      ...
      if kind = 0 then IT.terminal (symbol - 1) else IT.nonterminal symbol
    ...
    let incoming_symbol (s : 'a IT.lr1state) : 'a IT.symbol =
      ...
      match symbol with IT.X symbol -> Obj.magic symbol
    ...
    let rhs prod = List.map decode_symbol (read_packed_linearized IT.rhs prod)
    ...
    let export t : item = (t lsr 7, t mod 128)
    ...
    let nullable nt = decode_bool (PackedIntArray.get1 IT.nullable (n2i nt))
    ...
    let foreach_terminal f accu =
      let n, _ = TT.error in
      foldij 0 n (fun i accu -> f (IT.terminal i) accu) accu

    let foreach_terminal_but_error f accu =
      let n, _ = TT.error in
      foldij 0 n
        (fun i accu ->
          if i = TT.error_terminal then accu else f (IT.terminal i) accu)
        accu
    ...
    let feed_failure () = invalid_arg "feed: outgoing transition does not exist"
    ...
    let feed_nonterminal (nt : nonterminal) startp (semv : semantic_value) endp
        (env : 'b env) : 'b env =
      ...
      | Some target ->
          ...
          let stack =
            { state = source; semv; startp; endp; next = env.stack }
          in
    ...
    let feed_terminal (terminal : terminal) startp (semv : semantic_value) endp
        (env : 'b env) : 'b env =
...
module TableInterpreter = struct
  ...
  module MakeEngineTable (T : TableFormat.TABLES) = struct
    ...
    let foreach_terminal f accu =
      let n, _ = T.error in
      foldij 0 n (fun i accu -> f i accu) accu
    ...
    let unmarshal2 table i j =
      RowDisplacement.getget PackedIntArray.get PackedIntArray.get table i j
    ...
    let action state terminal value shift reduce fail env =
      ...
      | 1 ->
          ...
          let opcode = action land 0b11 and param = action lsr 2 in
          if opcode >= 0b10 then
            (* 0b10 : shift/discard *)
    ...
    let goto_prod state prod = goto_nt state (PackedIntArray.get T.lhs prod)
    ...
    module Log = struct
      ...
      let reduce_or_accept prod =
        match T.trace with
        | Some (_, productions) -> fprintf stderr "%s\n%!" productions.(prod)
        | None -> ()
      ...
      let lookahead_token token startp endp =
        ...
        | Some (terminals, _) ->
            fprintf stderr "Lookahead token is now %s (%d-%d)\n%!"
              terminals.(token) startp.Lexing.pos_cnum endp.Lexing.pos_cnum
        | None -> ()
      ...
      let handling_error state =
        match T.trace with
        | Some _ -> fprintf stderr "Handling error in state %d\n%!" state
        | None -> ()
    end
  end
end
