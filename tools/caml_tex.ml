...
module Output = struct
  ...
  type status = Ok | Warning of int | Error
...
module Text_transform = struct
  type kind = Underline | Ellipsis
  type t = { kind : kind; start : int; stop : int }

  exception Intersection of { line : int; file : string; left : t; right : t }
  ...
  let check_partition line file l =
    ...
    let check acc = function
      ...
      | U (u, n) ->
          ...
          u
    in
    List.fold_left ~f:check ~init (partition l) |> ignore
...
let process_file file =
  ...
  try
    while true do
      ...
      incr_phrase_start ();
      if string_match re_start !input 0 then (
        ...
        let mode =
          ...
          | _ -> assert false
        in
        if mode = Signature && not omit_answer then
          raise
        ...
        let read_phrase () =
          ...
          let rec read () =
            ...
            in
            let implicit_stop =
              if string_match ~!"\\\\end{caml_example\\*?}[ \t]*$" input 0 then
                if !phrase_stop = 1 + !phrase_start then raise End_of_file
        ...
        with End_of_file ->
          phrase_start := !phrase_stop;
          stop tex_fmt main)
      else if string_match ~!"\\\\begin{caml_eval}[ \t]*$" !input 0 then (
  ...
  | Incompatible_options (Signature_with_visible_answer (file, line_number)) ->
      fatal
        "when parsing a caml_example environment in@ %s, line %d:@,\
         the signature mode is only compatible with \"caml_example*\"@ Hint: \
  ...
  | Text_transform.Intersection { line; file; left; right } ->
      fatal
        "when evaluating a caml_example environment in %s, line %d:@ Textual \
         ...
         %d-%d interval.@ Hind: did you try to elide a code fragment which \
         raised a warning?"
  ...
  | Ellipsis.Nested_ellipses { first; second } ->
      fatal
        "when evaluating a caml_example environment,@ there were two nested \
         ellipsis attribute.@ The first one started at position %d,@ the \
         second one at %d"
        first second
