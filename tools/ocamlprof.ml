...
let add_incr_counter modul (kind, pos) =
  ...
  match kind with
  | Open ->
      fprintf !outchan "(%sProfiling.incr %s%s_cnt %d; " modprefix idprefix
        modul !prof_counter;
      incr prof_counter
...
let insert_profile rw_exp ex =
  ...
  and gh = ex.pexp_loc.loc_ghost in
  if gh || st = en then rw_exp true ex
  else (
    insert_action st en;
    rw_exp false ex)
...
let init_rewrite modes mod_name =
  ...
  if !instr_mode then (
    ...
    fprintf !outchan
      " 0;; Profiling.counters := (\"%s\", (\"%s\", %s%s_cnt)) :: \
       !Profiling.counters;; "
...
and rewrite_cases iflag l =
  List.iter
    (fun pc ->
      (match pc.pc_guard with None -> () | Some g -> rewrite_exp iflag g);
      rewrite_exp iflag pc.pc_rhs)
    l

and rewrite_labelexp_list iflag l = rewrite_exp_list iflag (List.map snd l)
and rewrite_exp_list iflag l = List.iter (rewrite_exp iflag) l

and rewrite_exp iflag sexp =
  if iflag then insert_profile rw_exp sexp else rw_exp false sexp
...
and rw_exp iflag sexp =
  ...
  | Pexp_function caselist ->
      if !instr_fun then rewrite_function iflag caselist
      else rewrite_cases iflag caselist
  | Pexp_fun (_, _, p, e) ->
      let l = [ { pc_lhs = p; pc_guard = None; pc_rhs = e } ] in
      if !instr_fun then rewrite_function iflag l else rewrite_cases iflag l
  ...
  | Pexp_tuple sexpl -> rewrite_exp_list iflag sexpl
  | Pexp_construct (_, None) -> ()
  | Pexp_construct (_, Some sarg) -> rewrite_exp iflag sarg
  | Pexp_variant (_, None) -> ()
  | Pexp_variant (_, Some sarg) -> rewrite_exp iflag sarg
  | Pexp_record (lid_sexp_list, None) ->
      rewrite_labelexp_list iflag lid_sexp_list
  | Pexp_record (lid_sexp_list, Some sexp) ->
      rewrite_exp iflag sexp;
      rewrite_labelexp_list iflag lid_sexp_list
  | Pexp_field (sarg, _) -> rewrite_exp iflag sarg
  | Pexp_setfield (srecord, _, snewval) ->
      rewrite_exp iflag srecord;
      rewrite_exp iflag snewval
  | Pexp_array sargl -> rewrite_exp_list iflag sargl
  ...
  | Pexp_while (scond, sbody) ->
      rewrite_exp iflag scond;
      if !instr_loops && not sexp.pexp_loc.loc_ghost then
  ...
  | Pexp_constraint (sarg, _) | Pexp_coerce (sarg, _, _) ->
      rewrite_exp iflag sarg
  | Pexp_send (sobj, _) -> rewrite_exp iflag sobj
  | Pexp_new _ -> ()
  | Pexp_setinstvar (_, sarg) -> rewrite_exp iflag sarg
  | Pexp_override l -> List.iter (fun (_, sexp) -> rewrite_exp iflag sexp) l
  | Pexp_letmodule (_, smod, sexp) ->
      rewrite_mod iflag smod;
      rewrite_exp iflag sexp
  | Pexp_letexception (_cd, exp) -> rewrite_exp iflag exp
  | Pexp_assert cond -> rewrite_exp iflag cond
  | Pexp_lazy expr -> rewrite_exp iflag expr
  | Pexp_poly (sexp, _) -> rewrite_exp iflag sexp
  | Pexp_object cl -> List.iter (rewrite_class_field iflag) cl.pcstr_fields
...
and rewrite_ifbody iflag ghost sifbody =
  if !instr_if && not ghost then insert_profile rw_exp sifbody
  else rewrite_exp iflag sifbody
  ...
  List.iter
    (function
      | { pc_guard = Some scond; pc_rhs = sbody } ->
          ...
          insert_profile rw_exp sbody
      | { pc_rhs = { pexp_desc = Pexp_constraint (sbody, _) } }
      (* let f x : t = e *) ->
          insert_profile rw_exp sbody
...
and rewrite_funmatching l = rewrite_annotate_exp_list l
and rewrite_trymatching l = rewrite_annotate_exp_list l
...
and rewrite_class_field iflag cf =
  ...
  | Pcf_method
      ( _,
        _,
        Cfk_concrete (_, ({ pexp_desc = Pexp_function _ | Pexp_fun _ } as sexp))
      ) ->
      rewrite_exp iflag sexp
  ...
  | Pcf_initializer sexp -> rewrite_exp iflag sexp
...
and rewrite_class_expr iflag cexpr =
  ...
  | Pcl_constr _ -> ()
  | Pcl_structure st -> List.iter (rewrite_class_field iflag) st.pcstr_fields
  | Pcl_fun (_, _, _, cexpr) -> rewrite_class_expr iflag cexpr
  ...
  | Pcl_let (_, spat_sexp_list, cexpr) ->
      rewrite_patexp_list iflag spat_sexp_list;
      rewrite_class_expr iflag cexpr
  | Pcl_open (_, cexpr) | Pcl_constraint (cexpr, _) ->
      rewrite_class_expr iflag cexpr
  | Pcl_extension _ -> ()

and rewrite_class_declaration iflag cl = rewrite_class_expr iflag cl.pci_expr
...
and rewrite_str_item iflag item =
  ...
  | Pstr_module x -> rewrite_mod iflag x.pmb_expr (* todo: Pstr_recmodule?? *)
...
let process_impl_file filename =
  ...
  if !instr_mode then (
    ...
    let allcounters = (input_value ic : (string * (string * int array)) list) in
    close_in ic;
    let modes, cv =
      try List.assoc modname allcounters
      with Not_found ->
...
let process_anon_file filename =
  if Filename.check_suffix filename ".ml" then process_impl_file filename
  else process_intf_file filename
