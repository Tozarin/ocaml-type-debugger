...
let print_cmx_infos (ui, crc) =
  print_general_infos ui.ui_name crc ui.ui_defines ui.ui_imports_cmi
    ui.ui_imports_cmx;
  (match ui.ui_export_info with
  | Clambda approx ->
      if not !no_approx then (
  ...
  | Flambda export ->
      ...
      if not !no_approx then (
        ...
        Format.printf "approximations@ %a@.@." Export_info.print_approx
          (export, root_symbols));
      if not !no_code then
        Format.printf "functions@ %a@.@." Export_info.print_functions export);
  let pr_funs _ fns = List.iter (fun arity -> printf " %d" arity) fns in
  ...
  List.iter
    (fun ui ->
      print_general_infos ui.dynu_name ui.dynu_crc ui.dynu_defines
        ui.dynu_imports_cmi ui.dynu_imports_cmx)
    header.dynu_units
...
let dump_obj filename =
  ...
  | Error head_error -> (
      ...
      | Ok () -> ()
      | Error () ->
          if Filename.check_suffix filename ".cmxs" then dump_cmxs ic
