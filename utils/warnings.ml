...
let descriptions =
  [
    ...
    (6, "Label omitted in function application.", [ "labels-omitted" ]);
    (7, "Method overridden.", [ "method-override" ]);
    (8, "Partial match: missing cases in pattern-matching.", [ "partial-match" ]);
    ( 9,
    ...
    ( 11,
      ...
      [ "redundant-case" ] );
    (12, "Redundant sub-pattern in a pattern-matching.", [ "redundant-subpat" ]);
    (13, "Instance variable overridden.", [ "instance-variable-override" ]);
    ( 14,
      "Illegal backslash escape in a string constant.",
      [ "illegal-backslash" ] );
    (15, "Private method made public implicitly.", [ "implicit-public-methods" ]);
    (16, "Unerasable optional argument.", [ "unerasable-optional-argument" ]);
    (17, "Undeclared virtual method.", [ "undeclared-virtual-method" ]);
    (18, "Non-principal type.", [ "not-principal" ]);
    (19, "Type without principality.", [ "non-principal-labels" ]);
    (20, "Unused function argument.", [ "ignored-extra-argument" ]);
    (21, "Non-returning statement.", [ "nonreturning-statement" ]);
    (22, "Preprocessor warning.", [ "preprocessor" ]);
    (23, "Useless record \"with\" clause.", [ "useless-record-with" ]);
    ( 24,
    ...
    ( 31,
      ...
      [ "module-linked-twice" ] );
    (32, "Unused value declaration.", [ "unused-value-declaration" ]);
    (33, "Unused open statement.", [ "unused-open" ]);
    (34, "Unused type declaration.", [ "unused-type-declaration" ]);
    (35, "Unused for-loop index.", [ "unused-for-index" ]);
    (36, "Unused ancestor variable.", [ "unused-ancestor" ]);
    (37, "Unused constructor.", [ "unused-constructor" ]);
    (38, "Unused extension constructor.", [ "unused-extension" ]);
    (39, "Unused rec flag.", [ "unused-rec-flag" ]);
    (40, "Constructor or label name used out of scope.", [ "name-out-of-scope" ]);
    (41, "Ambiguous constructor or label name.", [ "ambiguous-name" ]);
    ( 42,
      "Disambiguated constructor or label name (compatibility warning).",
      [ "disambiguated-name" ] );
    (43, "Nonoptional label applied as optional.", [ "nonoptional-label" ]);
    ( 44,
    ...
    (46, "Error in environment variable.", [ "bad-env-variable" ]);
    (47, "Illegal attribute payload.", [ "attribute-payload" ]);
    ( 48,
      "Implicit elimination of optional arguments.",
      [ "eliminated-optional-arguments" ] );
    (49, "Absent cmi file when looking up module alias.", [ "no-cmi-file" ]);
    (50, "Unexpected documentation comment.", [ "unexpected-docstring" ]);
    ( 51,
      "Function call annotated with an incorrect @tailcall attribute",
      [ "wrong-tailcall-expectation" ] );
    (52, "Fragile constant pattern.", [ "fragile-literal-pattern" ]);
    (53, "Attribute cannot appear in this context.", [ "misplaced-attribute" ]);
    ( 54,
      "Attribute used more than once on an expression.",
      [ "duplicated-attribute" ] );
    (55, "Inlining impossible.", [ "inlining-impossible" ]);
    ( 56,
    ...
    (58, "Missing cmx file.", [ "no-cmx-file" ]);
    ( 59,
      "Assignment to non-mutable value.",
      [ "flambda-assignment-to-non-mutable-value" ] );
    (60, "Unused module declaration.", [ "unused-module" ]);
    ( 61,
      "Unboxable type in primitive declaration.",
      [ "unboxable-type-in-prim-decl" ] );
    (62, "Type constraint on GADT type declaration.", [ "constraint-on-gadt" ]);
    (63, "Erroneous printed signature.", [ "erroneous-printed-signature" ]);
    ( 64,
    ...
    ( 65,
      ...
      [ "redefining-unit" ] );
    (66, "Unused open! statement.", [ "unused-open-bang" ]);
    (67, "Unused functor parameter.", [ "unused-functor-parameter" ]);
    ( 68,
...
let set_alert ~error ~enable s =
  let upd =
    ...
    | s ->
        ...
        in
        let f =
          if enable = pos then Misc.Stdlib.String.Set.add
          else Misc.Stdlib.String.Set.remove
        in
        (f s set, pos)
  in
  if error then current := { !current with alert_errors = upd }
  ...
  and loop_letter_num myset i =
    if i >= String.length s then error ()
      ...
      | '0' .. '9' ->
          ...
          for n = n1 to min n2 last_warning_number do
            myset n
          done;
          loop i
      | 'A' .. 'Z' ->
...
let message = function
  ...
  | Module_linked_twice (modname, file1, file2) ->
      Printf.sprintf "files %s and %s both define a module named %s" file1 file2
        modname
  ...
  | Ambiguous_var_in_pattern_guard vars ->
      ...
      Printf.sprintf
        "Ambiguous or-pattern variables under guard;\n\
         %s may match different arguments. %t" msg ref_manual_explanation
  | No_cmx_file name ->
      Printf.sprintf
        "no cmx file was found in path for module %s, and its interface was \
         not compiled with -opaque"
        name
...
let id_name w =
  ...
  | Some (_, _, s :: _) -> Printf.sprintf "%d [%s]" n s
  | _ -> string_of_int n
      ...
      *)
      let sub_locs =
        if (not alert.def.loc_ghost) && not alert.use.loc_ghost then
          [ (alert.def, "Definition"); (alert.use, "Expected signature") ]
        else []
      in
