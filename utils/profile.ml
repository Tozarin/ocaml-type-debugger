...
module Measure = struct
  type t = { time : float; allocated_words : float; top_heap_words : int }
...
let rows_of_hierarchy hierarchy measure_diff initial_measure columns =
  ...
  let make_row prev_top_heap_words (p : Measure_diff.t) ~toplevel_other =
    ...
    ( List.map
        (function
          | `Time -> make p.duration ~f:time_display
          | `Alloc -> make p.allocated_words ~f:memory_word_display
          | `Top_heap ->
              make
                (float_of_int p.top_heap_words_increase)
                ~f:memory_word_display
          | `Abs_top_heap ->
              make
...
let display_rows ppf rows =
  ...
  let display_cell i (_, c) ~width =
    ...
    ( display_cell,
      if display_cell then c.to_string ~max:maxs.(i) ~width
