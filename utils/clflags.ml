...
let pic_code =
  ref (match Config.architecture with (* -fPIC *)
      | "amd64" -> true | _ -> false)
...
let unsafe_string =
  if Config.safe_string then ref false else ref (not Config.default_safe_string)
...
let rounds () =
  match !simplify_rounds with None -> !default_simplify_rounds | Some r -> r
...
let inline_branch_cost = ref (Int_arg_helper.default default_inline_branch_cost)
...
let set_int_arg round (arg : Int_arg_helper.parsed ref) default value =
  let value : int = match value with None -> default | Some value -> value in
  match round with
  | None ->
      arg :=
        Int_arg_helper.set_base_default value
          (Int_arg_helper.reset_base_overrides !arg)
  | Some round -> arg := Int_arg_helper.add_base_override round value !arg

let set_float_arg round (arg : Float_arg_helper.parsed ref) default value =
  let value = match value with None -> default | Some value -> value in
  match round with
  | None ->
      arg :=
        Float_arg_helper.set_base_default value
          (Float_arg_helper.reset_base_overrides !arg)
  | Some round -> arg := Float_arg_helper.add_base_override round value !arg
...
let use_inlining_arguments_set ?round (arg : inlining_arguments) =
  ...
  set_int inline_branch_cost default_inline_branch_cost arg.inline_branch_cost;
  set_int inline_indirect_cost default_inline_indirect_cost
    arg.inline_indirect_cost;
  set_int inline_lifting_benefit default_inline_lifting_benefit
    arg.inline_lifting_benefit;
  set_float inline_branch_factor default_inline_branch_factor
    arg.inline_branch_factor;
  set_int inline_max_depth default_inline_max_depth arg.inline_max_depth;
  set_int inline_max_unroll default_inline_max_unroll arg.inline_max_unroll;
  set_float inline_threshold default_inline_threshold arg.inline_threshold;
  set_int inline_toplevel_threshold default_inline_toplevel_threshold
    arg.inline_toplevel_threshold
...
let set_dumped_pass s enabled =
  if List.mem s !all_passes then
    let passes_without_s = List.filter (( <> ) s) !dumped_passes_list in
    let dumped_passes =
      if enabled then s :: passes_without_s else passes_without_s
    in
...
module Compiler_ir = struct
  ...
  let extension t =
    let ext = match t with Linear -> "linear" in
    ".cmir-" ^ ext
  ...
  let extract_extension_with_pass filename =
    ...
    if ext_len <= 0 then None
      ...
      match ir with None -> None | Some ir -> Some (ir, drop_prefix ir)
end
...
module Compiler_pass = struct
  ...
  *)
  ...
  let passes = [ Parsing; Typing; Scheduling; Emit ]
  let is_compilation_pass _ = true
  let is_native_only = function Scheduling -> true | Emit -> true | _ -> false
  let enabled is_native t = (not (is_native_only t)) || is_native
  let can_save_ir_after = function Scheduling -> true | _ -> false

  let available_pass_names ~filter ~native =
    passes
    |> List.filter (enabled native)
    |> List.filter filter |> List.map to_string
...
let should_save_ir_after pass = List.mem pass !save_ir_after

let set_save_ir_after pass enabled =
  let other_passes = List.filter (( <> ) pass) !save_ir_after in
  let new_passes = if enabled then pass :: other_passes else other_passes in
    ...
    (function
      | (arg_name, _, _) as arg -> (
          try
            ...
            Printf.eprintf "Warning: compiler argument %s is already defined:\n"
              arg_name;
...
let print_arguments usage = Arg.usage !arg_spec usage
