...
module C = struct
  ...
  let map sub c =
    match c with
    | Pconst_integer _ | Pconst_char _ | Pconst_float _ -> c
...
module T = struct
  ...
  let map_type_extension sub
    ...
    Te.mk ~loc ~attrs (map_loc sub ptyext_path)
  ...
  let map_extension_constructor sub
    ...
    Te.constructor ~loc ~attrs (map_loc sub pext_name)
...
module CE = struct
  ...
  let class_infos sub f
    ...
    Ci.mk ~loc ~attrs ~virt:pci_virt
      ~params:(List.map (map_fst (sub.typ sub)) pl)
      (map_loc sub pci_name) (f pci_expr)
end
  ...
  let extension_of_sub sub =
    ( { loc = sub.loc; txt = "ocaml.error" },
      PStr
        [
          Str.eval
            (Exp.constant (Pconst_string (str_of_pp sub.txt, sub.loc, None)));
        ] )
  in
  ( { loc = main.loc; txt = "ocaml.error" },
    PStr
      (Str.eval
         (Exp.constant (Pconst_string (str_of_pp main.txt, main.loc, None)))
...
let get_cookie k =
  try Some (String.Map.find k !cookies) with Not_found -> None
  ...
  let get_fields = function
    | PStr
        [
          {
            pstr_desc =
              Pstr_eval ({ pexp_desc = Pexp_record (fields, None) }, []);
          };
        ] ->
        fields
  ...
  let restore fields =
    let field name payload =
      ...
      and get_bool pexp =
        ...
        | _ ->
            raise_errorf
              "Internal error: invalid [@@@ocaml.ppx.context { %s }] bool \
               syntax"
              name
      and get_list elem = function
        ...
        | _ ->
            raise_errorf
              "Internal error: invalid [@@@ocaml.ppx.context { %s }] list \
               syntax"
              name
      and get_pair f1 f2 = function
        ...
        | _ ->
            raise_errorf
              "Internal error: invalid [@@@ocaml.ppx.context { %s }] pair \
               syntax"
              name
      and get_option elem = function
        ...
        | _ ->
            raise_errorf
              "Internal error: invalid [@@@ocaml.ppx.context { %s }] option \
               syntax"
              name
      in
      match name with
      ...
      | "load_path" -> Load_path.init (get_list get_string payload)
      | "open_modules" -> Clflags.open_modules := get_list get_string payload
      | "for_package" -> Clflags.for_package := get_option get_string payload
      ...
      | "transparent_modules" -> Clflags.transparent_modules := get_bool payload
      | "unboxed_types" -> Clflags.unboxed_types := get_bool payload
      | "unsafe_string" -> Clflags.unsafe_string := get_bool payload
      | "cookies" ->
          let l = get_list (get_pair get_string (fun x -> x)) payload in
          cookies :=
            List.fold_left
              (fun s (k, v) -> String.Map.add k v s)
              String.Map.empty l
      | _ -> ()
    in
    List.iter
  ...
  let implem ast =
    let fields, ast =
      ...
      | {
          pstr_desc =
            Pstr_attribute
              { attr_name = { txt = "ocaml.ppx.context" }; attr_payload = x };
        }
        :: l ->
