...
let error_of_extension ext =
  let submessage_from main_loc main_txt = function
    | {
        pstr_desc =
          Pstr_extension (({ txt = "ocaml.error" | "error"; loc }, p), _);
      } -> (
        match p with
        | PStr
            [
    ...
    | { pstr_desc = Pstr_extension (({ txt; loc }, _), _) } ->
        {
          Location.loc;
          txt =
            (fun ppf -> Format.fprintf ppf "Uninterpreted extension '%s'." txt);
        }
    | _ ->
        {
...
let warning_attribute ?(ppwarning = true) =
  let process loc txt errflag payload =
    ...
    | None -> warn_payload loc txt "A single string literal is expected"
  in
